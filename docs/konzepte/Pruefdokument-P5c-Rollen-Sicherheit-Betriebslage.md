...
| **Eine echte Sicherung von vor 21.0.0 einspielen** (AP8) | Gemessen sind der Kreislauf `edbak-alt` gegen eine Altsicherung des Referenzbestands (Nutzlast 7) und Teil 13 der Wiederherstellungsprobe mit einer gestellten Nutzlast 11 samt Standortauswahl. Eine Datei, die jemand vor dem Deploy auf Staging gezogen hat, hat noch niemand eingespielt. | P-P5c-37 |
...
| Jobprobe | `proben.sh jobs` | in der Sandbox **36 / 1**, neu „`uebergangen` ist mit dem FTP-Weg gefallen" grün. Rot ist „Auslöser `anfrage` räumt den Rückstand ab" (10 Zeilen + 1 Blob → 5 + 0) — bei 2,5 Mio. Spurpunkten im Bestand der Sandbox, und mit dem Code von AP7 genauso. Auf frischer Anlage entscheidet der Prüfstand |
...
| **Prüfstand** | `hochfahren.sh --neu`, `pruefen.sh` | *steht in der Commit-Nachricht von `P5c-AP8`* |
...
| P-P5c-36 | **Ein Tag mit „Anderem Rettungsmittel"** (AP8, Nr. 169) | auf Staging, einmal am Handy: Diensttag anlegen → Rettungsmittel „Anderes Rettungsmittel …" → Bezeichnung eintippen, Typ „Standard", Betriebsart **luftgebunden**; Besatzung ansehen; dann auf **bodengebunden** umschalten, wieder ansehen; speichern; einen Einsatz an diesem Tag anlegen | bei luftgebunden sofort **Pilot 1, Pilot 2, HEMS-TC, Flugretter, Sonstige**, bei bodengebunden **Fahrer, Praktikant, Sonstige**; nach dem Speichern dieselben Rollen am Tag und im Einsatzformular; ein Tag dieser Art von vor dem Deploy trägt nach `update.php` die Rollen seiner Betriebsart | keine Besatzungsfelder am Tag oder im Einsatz (der Stand vor 21.0.0); die Rollen der falschen Betriebsart; auf dem Handy eine waagerecht schiebbare Seite | offen |
| P-P5c-37 | **Eine Sicherung von vor dem Deploy einspielen** (AP8) | **vor** dem Merge auf Staging an einem Testkonto: Einstellungen → Backup, Datei herunterladen; nach Merge und `update.php` dieselbe Datei im selben Konto einspielen | Einspielen ohne Fehler, die Rückmeldung zählt Standorte, Fahrzeuge und Einträge wie gewohnt; alle eigenen Standorte stehen in den Auswahllisten, auch die, die früher nicht „ausgewählt" waren; ein neues Backup danach lässt sich wieder einspielen | „Fassung nicht unterstützt" oder ein Abbruch; fehlende Stammdaten; ein Standort, der in der Auswahl fehlt | offen |
...
- **Teil 13 der Wiederherstellungsprobe baut die Nutzlast 11 selbst** — aus
  ...
  (Nutzlast 7) und P-P5c-37 mit einer von Staging.
...
- **Die Jobprobe war in der Sandbox rot an einer Stelle, die AP8 nicht
  ...
  AP7; ob er auf frischer Anlage verschwindet, sagt der Bericht im Commit.
