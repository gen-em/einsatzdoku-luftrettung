...
| **Die Leiste an einem echten Laptopfenster** (AP9, F-P5c-164, seit 21.1.3 behoben) | **Behoben mit 21.1.3** (nach dem Merge von P5c): Die Sprungmarken fallen jetzt unter 950 statt unter 800 px weg (Entscheidung der Betreiberin). Gemessen in Chromium bei 1280 px Breite und acht Höhen von 800 bis 1000 px: vorher bei 800 px 11 von 14 Seiten vollständig, bei 900 px 13 von 14; nachher überall 14 von 14; die Kante bei 949 / 950 px (0 bzw. 8 Marken). Nicht gemessen: ein echtes Laptopfenster mit Fensterrahmen und Browserleisten, Firefox und WebKit. | P-P5c-43 |
...
| **Prüfstand, Abschluss — dritter Lauf, angehalten** | nach der Behebung von Nr. 328 | nach der ersten Probe angehalten: eine Kommentarzeile in `komplett_lib.php` war noch umzubrechen, und ein Bericht über einen Baum, der sich danach ändert, belegt nichts |
| **Prüfstand, Abschluss — vierter Lauf, nicht verwendet** | derselbe Aufruf, frische Anlage (Zwischenstand `00cacef`) | **1 rot, 0 nicht gemessen, 49 grün, 2 603 s** — die Komplettprobe jetzt grün; rot war die **Bedienprobe, 57 von 58**: `ap5-standort-loeschen-variante-b` fand nach dem Löschen des Standorts keine Zielzeile („Adresse #veh-37, Zeile „", Fläche —, Karte offen false, 5 Einträge") |
...
| **Prüfstand, achter Lauf** | frische Anlage, nach F-P5c-172 | **50 grün, 0 rot, 0 nicht gemessen, 2 544 s**, im Commit `6456d7f`; Stufe 1 auf dem PR **grün** |
| **F-P5c-164 behoben — Höhenschwelle 950 statt 800 px** (Entscheidung der Betreiberin, 26.09.2026) | Messmittel aus AP9 (Playwright, BetreiberIn, 14 Seiten, 18 Einträge; gezählt: im Bild oder Kopf der zugeklappten Gruppe) bei 1280 px und acht Höhen, vorher gegen das Stylesheet von heute, nachher mit untergeschobenem Stylesheet | vorher: 800 px **11/14**, 835 px **12/14**, 891 und 900 px **13/14** (Servereinstellungen 16/18), 946–1000 px 14/14; nachher: **14/14 bei allen acht Höhen**. Kante: 1280 × 949 **0** sichtbare Marken, 1280 × 950 **8**; 1023 × 800 (Schublade) und 1920 × 1080 **8**. Stilvergleich: zwei ungeplante Signaturen (`div.eintrag-unterliste` display/height/width, `div.leiste-gruppe` height — er misst bei 900 px Höhe), mit `gegen.sh --schreiben` in `geplant.txt`, Diff genau diese zwei Zeilen; danach **172 geplant, 172 gemessen, 0 ungeplant**. Die erzeugte Schwellentabelle in `Design.md` 7 wortgleich zur Ausgabe von `design.py schwellen` |
| **PR #89 gemergt** | `29cf394`, 26.09.2026, mit dem Kopf `6456d7f` (Baum `328e119`, gleich dem Baum von `main`) | die Höhenschwelle war da noch nicht dabei; sie kommt als Web **21.1.3** in einem eigenen Pull Request |
| **Stilvergleich gegen das neue `main`** | `gegen.sh --schreiben`, dann `gegen.sh` | `geplant.txt` ohne die P5c-Zeilen (Rahmenplan 6 erledigt): **13 Signaturen**, alle aus der Höhenschwelle — sechsmal `display: flex -> none` an den Sprungmarken (Breiten ab 1024 px, gemessen bei 900 px Höhe), die Katalogseite **31,2 px** kürzer (`html`, `body`, `div`), die absolut gesetzten Elemente um denselben Betrag verschoben; **13 geplant, 13 gemessen, 0 ungeplant** |
| **Prüfstand, Web 21.1.3 — erster Lauf** | Stufe `klein`, frische Anlage | **23 grün, 0 rot, 0 nicht gemessen, 846 s**, im Commit `8c00950`; Stufe 1 auf PR #90 grün |
| **Nr. 330 — geerbte Zeilen im Stilvergleich** (auf Nachfrage der Betreiberin) | `stilvergleich.js --geerbt`, übergeben von `gegen.sh` aus dem Vergleichsstand | Gegenproben: Zustand nach dem Merge (Vergleich gegen den eigenen Stand) ohne `--geerbt` **rot, 13 nicht gemessen**, mit **grün, 13 geerbt**; eine eigene Zeile ohne Messung **rot (1)**; leere Liste gegen `main` **rot (13 ungeplant)**; der Pull-Request-Fall über `gegen.sh` **grün, 13 / 13**. `geplant.txt` mit `--schreiben` neu: nur der Kopf geändert, die 13 Signaturen gleich |
| **PR #90 gemergt** | `77f5ac8`, 26.09.2026, mit dem Kopf `8c00950` (Baum `57287b0`, gleich dem Baum von `main`) | Nr. 330 war da noch nicht dabei; es kommt als Werkzeugänderung ohne Versionsstufe auf dem neuen `main` |
| **Prüfstand, Nr. 330** | Stufe nach der Berührung, frische Anlage | *steht in der Commit-Nachricht des Kopf-Commits* — der Stilvergleich darin ist der erste Lauf nach einem Merge ohne Leeren: 13 Zeilen geerbt, 0 rot |
...
| P-P5c-43 | **Die Leiste an einem Laptop** (AP9, E-P5c-131, F-P5c-164; Schwelle seit 21.1.3 bei 950 px) | am eigenen Laptop mit einem Fenster um 850 px und einem über 950 px Höhe (Innenmaß des Browsers, nicht des Bildschirms): Betrieb → Servereinstellungen, dann Betrieb → Status | um 850 px: keine Sprungmarken in der Leiste, alle 18 Einträge ohne Rollen erreichbar; über 950 px: die Sprungmarken stehen, und die Leiste reicht trotzdem nicht über den Rand | um 850 px Sprungmarken sichtbar (dann ist das alte Stylesheet im Zwischenspeicher — neu laden) oder ein Eintrag nur mit Rollen erreichbar; über 950 px eine Leiste, die in sich rollt | offen |
