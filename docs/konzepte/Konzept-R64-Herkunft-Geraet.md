...
> | | |
> |---|---|
> | Stand | 04.09.2026 — **AP1, AP3, AP2 und AP4 erledigt** (Web 14.0.0, 14.1.0, 14.2.0, 14.2.1, 14.2.2). Entscheidungen E-R64-01 bis **E-R64-23**; keine offene F-Frage. Drei Funde am Code (F-R64-03, -04, -05), **alle drei behoben** |
> | Paket in Arbeit | **keines — AP1 bis AP5 sind fertig.** Offen ist nur die Freigabe des Auftraggebers: Erledigt-Zeile in Rahmenplan Abschnitt 8, Löschung dieses Konzepts (R62) und der Push auf `main` |
> | Erledigt | **AP1** — Datenmodell, Ableitung, anlegende Stellen, Migration `2026_09_04_herkunft_geraet`; Ingestprobe 39/0, Register 42 = 42 · **AP3** — Export und Anzeige; CSV-Kreislauf 8965/1023/5, Excel unverändert 31 Spalten · **AP2** — Nutzlast 9, Momentaufnahme und Sperrvermerke in der Sicherung; Wiederherstellungsprobe 94/0, edbak-alt-Kreislauf 0 unerklärt · **AP4** — Referenzbestand mit echt gekoppelten Geräten und einem Schnitt; alle sechs Herkunftswerte belegt; drei Kreisläufe **0/0/0** unerklärt · **AP5** — Vertrag 2.2, `version.php`, CHANGELOG, Technik, Handbuch, Export-Format, Backlog 63 und 83 nach *Erledigt*, Rahmenplan Fassung 27, Prüfdokument mit 15 Prüfpunkten |
...
> | Wo es hakt | nichts. F-R64-05 ist am 04.09.2026 entschieden und behoben (Web 14.2.2) |
...
| **F-R64-05** | **„kein Ende“ an einem abgeschlossenen Einsatz.** Die Einsatztabelle rechnete die Dauer aus Beginn und **Phase 9**; fehlte die, zeigte sie „kein Ende“ — auch bei `final = 1` und gesetztem `ended_at`. Betroffen sind geschnittene und importierte Einsätze (gemessen: 3). Dieselbe Rechnung stand an **vier** Stellen: `api/day.php`, `api/range.php`, `api/suchindex.php` und — über das Merkmal `has_p9` — die Einsatzansicht | **behoben** in Web 14.2.2 auf Entscheidung des Auftraggebers („Dauer bedeutet, wie lange der Einsatz gedauert hat“). Gerechnet wird jetzt aus `started_at` und `ended_at`; `has_p9` heißt `hat_ende`. **Gemessen, dass keine bestehende Zeile sich ändert:** über 330 aktive Einsätze fallen Phase 9 und `ended_at` **nullmal** auseinander (323 gleich, 3 mit Ende ohne Phase 9, 4 offen). Drei korrelierte Unterabfragen auf `mission_phases` sind dabei ersatzlos entfallen |
...
| AP5 | **erledigt** 04.09.2026 | — (nur `docs/`; die Codeänderung zu F-R64-05 zählt zu Web 14.2.2) | **P-8 nachgemessen statt gelesen:** Ein Archiv aus Web 14.1.0 (mit den zwei neuen Spalten) durch einen frischen CSV-Umlauf — Profil erkannt, „12 Einsätze, 0 Hinweise, 0 Fehler", **2011 Einzelvergleiche, 171 erwartete, 0 unerklärte** · JSON-Vertrag auf **Fassung 2.2**: `cut-` in der Präfix-Tabelle nachgetragen (**B-R64-01 damit behoben**), Spalte „Herkunft (`origin`)" je Präfix, Abschnitt zur Ableitung · **Prüfdokument angelegt** (`Pruefdokument-R64-Herkunft-Geraet.md`) mit allen Zahlen aus AP1 und AP3 und acht Prüfpunkten · Verweiszeile im Prüfdokument S4 | Nachgezogen: `version.php` (zwei Erzählungen), CHANGELOG, Technik.md, Handbuch 3.2 und 10.2b, Export-Format, Backup-Format, Backlog **63 und 83 nach *Erledigt***, Rahmenplan **Fassung 27** samt Fahrplanzeile, Prüfdokument mit **15** Prüfpunkten |
...
5a. **Und die Abnahme im Browser legte einen zweiten offen** (F-R64-05,
   oben). Der geschnittene Einsatz stand in der Tagesliste mit der Plakette
   „kein Ende" — die Dauer kam aus Phase 9, die er nicht hat. Zuerst nicht
   geändert, weil `api/day.php` das Verhalten im Kommentar ausdrücklich
   gewollt nannte; auf Entscheidung des Auftraggebers dann behoben (Web
   14.2.2). Es waren **vier** Stellen, nicht eine: Tagesliste,
   Zeitraumübersicht, Suche und — über `has_p9` — die Einsatzansicht.
