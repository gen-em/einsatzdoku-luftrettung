...
> | AP6 Abschluss: Dokumente, Backlog, Rahmenplan | **erledigt** 14.09.2026 | **keine** (nur `docs/`) | Rahmenplan Fassung **61**: Erledigt-Zeile in Abschnitt 8, Fahrplanzeile 9b auf gebaut, Kopf nachgezogen, **zwei** Zeilen nach Abschnitt 6 (Prüfliste P-01 bis P-14, Freigabe des Abschlusses), Abschnitt 5 berichtigt — Selbstprüfzahl **51 = 51** und jetzt **deckungsgleich** (`comm`: 0 Nummern Unterschied in beide Richtungen; vorher stimmte die Summe und die Mengen nicht, F-S10-AP6-01). `docs/Technik.md` 4.97c: `ftp` abgeschafft, sieben Wege am Altziel beschrieben; **drei veraltete Zahlen** berichtigt (Versandprobe 115 → 116, Komplettprobe 76 → 72, „drei Protokolle" → zwei). Backlog Nr. 140 mit Zuordnung **R40 (2)** statt S10. Wortliste **0/0/0**; Linkprobe **117/0**; `kdf_pepper` außerhalb von Vorbereitung und Review **0** |
...
| AP6 | Rahmenplan Abschnitt 5 gegen `docs/Backlog.md`, mit `comm` statt mit der Summe | Nummern nur hier / nur dort | **0** · **0** — vorher **1 · 1** (139 hier zu viel, 187 zu wenig) bei gleicher Summe 51 (F-S10-AP6-01) |
| AP6 | Selbstprüfzahl nach dem Einzeiler aus Fassung 46 | Abschnitt 5 / Backlog offen | **51 = 51**, und diesmal dieselben 51 |
| AP6 | `grep -rn "kdf_pepper" docs/ server/ CLAUDE.md` | Treffer außerhalb von Vorbereitung und Review | **0** (der Name aus der Vorbereitung ist nirgends in den Code gewandert) |
| AP6 | `grep` über `docs/Technik.md` nach `ftp` als **wählbarem** Protokoll | Stellen, die es noch als Option führen | **0** (vorher **5**: Adaptertabelle, „Ein Adapter für FTP **und** FTPS", die Tauglichkeitstabelle, „das einzige der **drei** Protokolle" und der Verzeichnisbaum — die fünfte fand erst die Gegenprobe nach dem ersten Durchgang) |
| AP6 | `tools/wortliste/` · `tools/linkprobe/` | nach der Dokumentationsarbeit | **0/0/0** · **117/0** |
| AP6 | **Konsistenzlesen Technik 7 ↔ Handbuch 12 ↔ Backup-Format 5** (die AP6-Abnahme verlangt es) | Widersprüche gefunden / behoben | **1 von 1**: Das Handbuch nannte in 12.6 **drei** Stücke des Wiederanlaufpakets, die Technik seit S10 **vier** — und das im selben Dokument, das den Server-Anteil in 12.5 längst beschreibt (F-S10-AP6-02) |
| AP6 | Tabellen der drei geänderten Dokumente nachgezählt | Zeilen mit abweichender Spaltenzahl, **neu** entstanden | **0** (die fünf gemeldeten sind maskierte `\|` in Zellen und standen vorher schon so da — Gegenlauf mit `git stash`) |
...
| F-S10-AP6-01 | **Die Selbstprüfzahl des Rahmenplans stand auf „51 = 51" und war trotzdem falsch.** Abschnitt 5 führte **Nr. 139** noch als offen — seit AP4 steht sie im Backlog unter *Erledigt* — und **Nr. 187** gar nicht, obwohl AP3 sie dort aufgenommen hat. Beide Listen zählten 51, und keine zwei davon waren dieselben 51: **zwei Fehler, die sich in der Summe aufhoben** | `docs/Rahmenplan.md` Abschnitt 5 | **Erledigt in AP6.** 139 ausgetragen, 187 eingetragen, danach mit `comm` gegengeprüft: **0 Nummern Unterschied** in beide Richtungen. *Der Kasten über der Tabelle verlangt genau das — „wer eine Zeile hinzufügt oder entfernt, rechnet sie im selben Zug nach" —, und der Einzeiler, den Fassung 46 dort hinterlegt hat, zählt nur. Eine Summe ist kein Mengenvergleich; zwei Fehler in verschiedene Richtungen sieht sie nie.* Nebenbei berichtigt: Nr. 140 trug die Zuordnung „Zuarbeit / **S10**", während R78 (7) das Deploy-Tor ausdrücklich an R40 (2) hängt — S10 hätte es nie bauen dürfen und hat es nicht |
| F-S10-AP6-02 | **Das Handbuch nannte drei Stücke des Wiederanlaufpakets, die Technik vier.** `docs/Handbuch.md` 12.6 schreibt seit Web 12.2.0: „Diese **drei** Dinge zusammen heissen Wiederanlaufpaket" — `config.php`, der Schlüssel darin, der Zugang zum Ziel. Seit S10 sind es **vier**, und `docs/Technik.md` 7 sagt das auch. Das Pikante: **Dasselbe Dokument beschreibt den Server-Anteil in 12.5 ausführlich** — die Stelle, die ihn hätte mitzählen müssen, war nur drei Bildschirmseiten entfernt und ist nicht mitgewandert | `docs/Handbuch.md` 12.6 | **Erledigt in AP6**, gefunden vom Konsistenzlesen, das die AP6-Abnahme ausdrücklich verlangt. Die vier Stücke stehen jetzt als Liste da, mit dem Satz, auf den es für BedienerInnen ankommt: **Ohne den Serverschlüssel ist ein Komplett-Backup endgültig zu.** Ohne den Server-Anteil kommt zunächst niemand an die geschützten Angaben — **verloren ist aber nichts**, jede NutzerIn kommt über ihren Wiederherstellungsschlüssel wieder herein. *Zwei Geheimnisse in einer Datei, zwei sehr verschiedene Folgen; wer sie in einem Wort zusammenfasst, verschenkt genau die Auskunft, die im Ernstfall zählt* |
