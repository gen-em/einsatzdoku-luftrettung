...
| **05/4** | Dokumente; dazwischen der Merge von PR #80 auf dem neuen Weg (`5671d24`, mit Bericht, P-PK-35) und F-PK-39; Prüfstand, Bericht im letzten Commit, PR | `6299836`, `19d8907`, `5671d24`, `6d6d58c`, letzter mit Bericht |
...
gemessen** — geschlossen wurde auf Verdacht. Gemessen ist an P-PK-31, dass
ein Handlauf jetzt gegenliest: auf einem Kopf ohne Bericht rot.
...
**E-PK-45 — die neun roten Proben der Nebenstufe werden vor P5c behoben**
(Auftraggeber, 23.09.2026, auf die Messung in Prüfdokument 5o). Die
Nebenstufe misst heute 36 Proben in 1 266 s, 9 davon rot (F-PK-40, Backlog
Nr. 292); mit Lage 5 käme kein Nebensprung durch das Tor. Ein eigenes
Korrekturpaket auf eigenem Zweig, nach dem PR von PK-05 und vor P5c AP1.
**Nicht gewählt:** eine Liste bekannter roter Proben, die Lage 5 ausnimmt —
sie wäre ein Filter vor dem Riegel.

**E-PK-46 — eine nicht gemessene Probe steht im Bericht und ist rot**
(Auftraggeber, 23.09.2026). Bis dahin fehlte sie dort ganz, und das Tor sah
sie nicht. Jetzt schreibt der Prüfstand `name=nicht-gemessen`, Lage 5 liest
es als rot, und der Lauf endet mit rc 1 (E-KH-12: Überspringen ist rot). Ein
Umgebungswert, den ein Aufruf nennt und der fehlt, ist eine fehlende
Voraussetzung wie ein fehlendes SDK — bis dahin stürzte der Aufruf mit
„unbound variable" ab. **Es zählen nur ausgewählte Proben:** Ein PR ohne
`android/` braucht kein SDK.

**E-PK-47 — die Schemaprobe wird Pflichtprüfung** (Auftraggeber, 23.09.2026,
Q-PK-07). „Schema gegen MySQL 8.4.0" und „Schema gegen MariaDB 10.6" kommen
als Required Checks in „Main Protect"; eine Migration, die auf einer der
beiden Datenbanken scheitert, lässt sich dann nicht mehr mergen. Gesetzt wird
es im Ruleset von der Betreiberin (Z12) — **bis dahin gilt es nicht.** Auf
`main` überspringt `Schon gemessen?` beide Jobs; übersprungen zählt dem
Ruleset als grün, das stört nicht.

**E-PK-48 — `CIQ_GERAETE_URL` bleibt stehen bis PK-08** (Auftraggeber,
23.09.2026, Q-PK-08). Kein Arbeitslauf liest das Repositoriumsgeheimnis seit
PK-05; PK-08 entscheidet, ob die Kette die Uhr für einen Tag baut.

**E-PK-49 — 337 Zeilen sind die Abnahme** (Auftraggeber, 23.09.2026,
Q-PK-09). Die Zahl 250 stammt aus der Zeit vor TB; die Abweichung ist oben
begründet.

**Zuschnitt des Korrekturpakets bestätigt** (Auftraggeber, 23.09.2026, zu
E-PK-45): (a) Verdrahtung des Prüfstands — `versandprobe`,
`spaltenregister-wegprobe`, `freigabeprobe`; (b) die csv-Referenz nach der
Hausform; (c) die fünf übrigen je einzeln — Anwendung falsch heißt
Korrekturstufe Web, Erwartung veraltet heißt Probe berichtigen; (d) die Dauer
der Nebenstufe. Vor Beginn ein kurzes Konzept mit Kürzel, Paketen und
Fächerungszeile, zur Bestätigung.

**Offen aus PK-05:** der Push-Weg auf `main` nach dem Merge (P-PK-32; die
Live-Abnahme P-PK-29 bis -31 ist an PR #81 gefahren), das Korrekturpaket für die
neun roten Proben (E-PK-45, Nr. 292) — **gebaut mit Konzept RP, PR #83** —, danach die erste echte Nebenstufe
(P-PK-33, mit P5c) und das Ruleset für die Schemaprobe (Z12). Die drei
Fragen Q-PK-07 bis -09 sind beantwortet (E-PK-47 bis -49).
...
| **F-PK-39** | PK-05 (Merge von PR #80) | **Im Merge-Zustand hielt der Prüfstand die Dateien aus `main` für berührt.** `git status` zeigt sie gegen `HEAD`; beim ersten Gang des Wegs aus `Pruefablauf.md` 5.3 fuhr er 18 statt 14 Proben. Mehr, nicht weniger — aber eine rote Probe aus fremdem Code hätte den Merge-Commit rot gemacht. | Behoben (`6d6d58c`): berührt ist, was im Arbeitsbestand wirklich anders ist als in der Basis, und jede neue Datei. Nachgestellt: alt 55 berührt (6 unter `server/`), neu 15 (0). |
| **F-PK-40** | PK-05 (Messung) | **Die Nebenstufe misst 36 Proben in 1 266 s, und 9 sind rot** — auf frischer Anlage, Web 20.37.3. Drei Verdrahtungsfehler des Prüfstands (`versandprobe`, `spaltenregister-wegprobe`, `freigabeprobe`), eine veraltete Referenz (`kreislauf-csv` nach der Hausform), fünf ungeklärte (`wiederherstellung` = F-PK-18, `gpxprobe`, `wartungsprobe`, `mailprobe`, `browserprobe-csp`). Bis PK-05 lief keine davon in einer Stufe, die ein Tor las. | E-PK-45: Korrekturpaket vor P5c, Backlog Nr. 292. Ziel 15 min verfehlt — die Dauer gehört in dasselbe Paket. **Erledigt mit Konzept RP:** 36 / 0 / 0, 1 239 s; Ziel rund 21 min (E-RP-05). |
| **F-PK-41** | PK-05 (nach dem Merge, P-PK-32) | **Ein übersprungener Matrix-Job meldet sich unter dem unaufgelösten Namen.** Auf `main` (`b329ac3`, Lauf 35891892887) hieß der übersprungene Schema-Job `Schema gegen ${{ matrix.db.name }}` — GitHub löst die Matrix nicht auf, wenn der Job gar nicht startet. | Für E-PK-47 unschädlich: Auf einem PR läuft der Job immer (`Schon gemessen?` sagt dort „messen") und heißt richtig. **Ins Ruleset gehören die aufgelösten Namen** (Z12, P-PK-36); der unaufgelöste ließe jeden PR auf eine Prüfung warten, die nie kommt. |

**Fragen an die Betreiberin aus PK-05:**

| Nr. | Frage | Empfehlung der Instanz |
|---|---|---|
| **Q-PK-07** | Soll `Schema gegen MySQL 8.4.0` / `Schema gegen MariaDB 10.6` Pflichtprüfung im Ruleset „Main Protect" werden? Heute hält ein roter Schemalauf keinen Merge auf; `Pruefablauf.md` 2 sagte bis PK-05 „Rot heißt kein Merge" für die ganze Station. | **Ja** — **beantwortet 23.09.2026: ja, beide** (E-PK-47; gesetzt wird es mit Z12) |
| **Q-PK-08** | Das Repositoriumsgeheimnis `CIQ_GERAETE_URL` liest seit PK-05 kein Arbeitslauf mehr. Löschen? | **Stehen lassen** bis PK-08 — **beantwortet 23.09.2026: stehen lassen** (E-PK-48) |
| **Q-PK-09** | `pruefung.yml` hat **337** Zeilen statt „unter 250". Reicht das als Abnahme? | **Ja** — **beantwortet 23.09.2026: 337 akzeptiert** (E-PK-49) |
