...
P-TB-07 abgehakt ist (P-TB-06 erfüllt am 23.09.2026). Nach `CLAUDE.md` 7: Was wurde
...
| P-TB-06 | **Beim nächsten Tag:** das Tor geht ohne Warten auf (TB-02) | Tag `web-v*` unmittelbar nach dem Merge setzen, Produktionslauf freigeben | Schritt 7 nennt ≥ 1 Lauf mit passendem Baum und Job `Stufe 1` grün; kein „Kein grüner Stufe-1-Lauf" | Schritt 7 rot mit derselben Meldung wie Lauf 35834341392 | **erfüllt 23.09.2026, Lauf 35852217360** (Tag `web-v20.37.2` auf `1139ce7`): Schritt 7 „50 Läufe geholt, 2 mit Baum c02047f, 0 Baum-Abrufe gescheitert"; `[JA] Lauf 35845602848 (pull_request, 9fcee9c): gleicher Baum, Job Stufe 1 grün`; der Verweis-Lauf 35846293553 auf `main` ausdrücklich `[nein]`. Auslieferung grün, Fassung gemeldet 20.37.2, Zeiger `produktion` auf `1139ce7`. Davor zwei Tags ohne passende Fassung (Läufe 35844281706, 35845324185) — an der Tag-Prüfung abgewiesen, Tor nicht erreicht |
...
| P-TB-06 | Lauf 35852217360 (`auslieferung.yml`, Tag `web-v20.37.2`, `1139ce7`), Schritt 7 | **50 Läufe, 2 mit Baum `c02047f`, 0 Abrufe gescheitert; 1 anerkannt** (PR-Lauf 35845602848), der Verweis-Lauf auf `main` (35846293553) **nicht** — genau E-TB-05. Staging-Lauf 35846294225 anerkannt, Abstammung `ahead`. `produktion / ausliefern` grün in 1 min 11 s, „Tag: web-v20.37.2 · erwartet: 20.37.2 · gemeldet: 20.37.2", Job `zeiger` grün. Die „ABBRUCH"-Zeilen im Protokoll gehören zur Selbstprobe von `tor.py`, nicht zum echten Backup-Tor | 23.09.2026 |
| Abschluss | Lauf 35846293553 (`pruefung.yml`, Push `main`, `1139ce7`) | zweiter Verweis: `Stufe 1` übersprungen, 14 s | 23.09.2026 |
