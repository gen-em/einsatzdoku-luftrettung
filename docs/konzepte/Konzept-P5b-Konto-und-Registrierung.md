...
> | Umsetzung | **fertig** auf `claude/magical-dirac-we2y1z` seit 16.09.2026 — begonnen auf dem Stand von `main` nach dem Merge von P5a (PR #50, `14f99ac`). **Alle zehn Pakete sind gebaut** (Tabelle unten); der Zweig ist am 17.09.2026 gegen `origin/main` gemergt und liegt zur Freigabe bereit. **Abweichend vom Fahrplan läuft 10b vor Schritt 15**: Der Auftraggeber hat die Umsetzung am 16.09.2026 unmittelbar nach dem P5a-Merge beauftragt; **Schritt 15 ist unverändert offen**. Welche Folgen das für die Zentralisierung hat, ist hier nicht gemessen — sie fasst dieselben Dateien an und misst ohnehin nach dem Merge neu (Rahmenplan, Fahrplanzeile 15). Reihenfolge und Abhängigkeiten innerhalb von 10b sonst wie in 3.0 |
...
> | AP3 — Registrierung | **erledigt** | Web 20.22.0 | `3f28e22` | Honeypot, Mindestausfülldauer 4 s, drei Rätsel-Töpfe; Wegwerfliste **8 von 8** Anbietern, **0 von 10** echten Domains; immer dieselbe Antwort. **20.22.1** berichtigte den Wortlaut der Häkchen, **20.22.2** hält sie fest (Nr. 231) |
...
> | AP9 — Onboarding und Rückfragen | **erledigt** | Web 20.24.0 | `978b345` | Die Abnahme ist der Schlüsselwechsel, im Browser gemessen: zweimal erneuert, dann **Code B gegen die neue Hülle: öffnet**, **Code A gegen die neue Hülle: öffnet nicht (OperationError)**, **Code A gegen seine alte Hülle: öffnet** — der Fehlschlag belegt damit die Ersetzung und nicht einen kaputten Erzeuger. Rundenzählung: `rueckfrage_naechste` 2027-03-17 = **6 Monate**, Runde **1**, verschoben **0**. Betreiber-Rückfrage: Positionen je Anzeige neu, Fehlversuch zählt (Meldung „Noch 2 Versuche"), Frage bleibt unverändert, Großschreibung angenommen, `schluesselblatt_bestaetigt_am` gesetzt, Sicherheitseintrag **ohne Werte**. Bilderlauf **24 Bilder, 0 Überlauf / 0 Konsolenfehler / 0 Knopfhöhen**; Wortliste **0 Treffer in 5 Bereichen**; Vollständigkeit **398** (+10, alle in drei neuen Dateien nachgezählt) |
> | AP10 — Abschluss | **erledigt** | Web 20.24.0 | `dde3956` | Merge von `origin/main` mit **drei Nummernkollisionen** (Web 20.16.0 → 20.16.5, Backlog 215–225 → 223–233, Fassung 77 → 78/79). Fünf Konfliktdateien von Hand aufgelöst. Prüfzahlen gegen den gemergten Stand: `php -l` **0** · Wortliste **0** in fünf Bereichen · Vollständigkeit **398** auf der Schwelle · Migrationsregister **0** · Kontraste **22/0** · Kettenaufrufe **0/0** · Tore **11/0** · CSP **0** · Sitzungshärtung **0** · Bilderlauf **16 Seiten, 128 Bilder, 0/0/0**, alle byteweise verschieden. Eine unabhängige Durchsicht fand danach **18 bestätigte Befunde**, darunter eine Nummer von `main`, die der Umnummerierungslauf mitverschoben hatte |
>
