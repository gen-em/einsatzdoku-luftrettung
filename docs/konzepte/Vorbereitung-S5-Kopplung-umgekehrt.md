...
| **B5.3** (bestätigt, **behoben**) | `ingest.php` schreibt beim Upsert `ended_at = VALUES(ended_at)` **bedingungslos**, `final` dagegen mit `GREATEST` | stimmt — und der Befund war zu klein: betroffen sind **drei** Spalten an `missions` (`ended_at`, `distance_m`, `ascent_m`) und eine an `rest_segments`. **Nachgestellt** gegen die laufende Installation: nach dem finalen Paket `ended_at 08:00 / distance_m 12345 / ascent_m 678 / final 1`, nach einem späteren nicht-finalen Paket `null / null / null / final 1` | **Behoben in Web 13.0.1** (`COALESCE(VALUES(x), x)` an allen vier Stellen), gehalten von Ingestprobe Teil 7 (30/30 statt 24/24). Für **E2** bleibt die Reihenfolge beim Nachsenden die richtige Zusage — der Server verzeiht sie jetzt nur. Nebenbei berichtigt: Am Ruhe-Segment zeigte sich der Fehler als Widerspruch, nicht als „läuft noch“ — `schneiden.js` 341–346 zieht das Ende aus `ended_at` und den Zusatz aus `final`, und `final` blieb ja 1 |
