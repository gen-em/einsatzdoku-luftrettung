...
|---|---|---|
| **PK-05: Stufe 1 auf GitHub, mit Bericht und ohne** | Die Arbeitsumgebung hat keinen Läufer. Örtlich nachgestellt sind der Gegenlese-Schritt (ohne Bericht rot, falscher Baum rot, passend grün) und der Umgebungswert-Schritt (leer grün, belegt rot); ob GitHub dieselben Ausdrücke so auswertet (`steps.*.outcome`, `head.sha \|\| github.sha`), zeigt erst ein Lauf. | am eigenen PR (P-PK-29, -30) |
