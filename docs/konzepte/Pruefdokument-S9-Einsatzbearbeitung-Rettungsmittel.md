...
| Bilderlauf | `seiten.json` **46 → 44 Seiten** (Gruppe Administration **9 → 7**). Gefahren in **beiden** Bedienhöhen, je **352 Einzelbilder und 44 Kontaktbögen**: **0 Überlauf / 0 Konsolenfehler / 0 falsche Knopfhöhen** (Zeiger 44/36 px, Finger 44 px). **0 ausgefallene Aufnahmen** — vorher fielen bei jedem Lauf **8** aus, weil `__ADMIN_STANDORT__` nicht auflösbar war (F-S9-U-33). Der Fund ist damit nicht nur erklärt, sondern erledigt |
