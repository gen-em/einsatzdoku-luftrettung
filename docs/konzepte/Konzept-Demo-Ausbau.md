...
| **E-DA-31** | **Der Demo-Ausbau zieht auf 197/198 und Fassung 67 nach, nicht `main` auf 192/193** (Nachlauf, Umsetzung). Beide Zweige haben am 14./15.09.2026 die Backlog-Nummern **190** und **191** und die Rahmenplan-Fassung **66** vergeben; der R42-Nachlauf war zuerst auf `main` (PR #47, `99d318a`). Wer zuerst auf `main` ist, behält seine Nummern — alles andere hieße, eine bereits veröffentlichte Nummer umzudeuten, und genau davor warnt der Rahmenplan in Abschnitt 5 („Wer auf einem Zweig eine Nummer vergibt, ohne den Stand des Rahmenplans zu kennen, vergibt sie zweimal"). Nachgezogen sind **alle** Fundstellen, auch die in `server/` (`api/range.php`, `version.php`). **Die Angabe in E-DA-19 war schon bei der Konzepterstellung überholt:** Sie nennt als höchste vergebene Nummer **186**; tatsächlich standen am 15.09.2026 bereits 187–189 im Backlog, und PR #47 hat noch am selben Tag bis 196 vergeben. |
| **E-DA-32** | **Die zweite Hälfte von Nr. 189 wird mitgenommen** (Nachlauf, Umsetzung). Der R42-Nachlauf hat Nr. 189 um einen zweiten Kommentarfehler ergänzt: An `geraet_modell` steht „der laengste hat 156 Zeichen", es sind **153**. AP0 hatte die erste Hälfte behoben und den Punkt nach *Erledigt* verschoben — nach dem Merge stünde er halb erledigt da. Die Umsetzung behebt deshalb auch die zweite Hälfte an den **drei lebenden** Stellen. Der Preis, den Fassung 65 gescheut hat (eine Versionsstufe für einen Kommentar), fällt hier nicht an: Web 20.3.0 ist noch nicht ausgeliefert. **Eigens nachgemessen** statt abgeschrieben: `mb_strlen()` über `GERAETE_MODELLE` ergibt 153 Zeichen (154 Bytes), 173 Modellnamen, fünf über 64 Zeichen. |
...
| **F-DA-6** | `server/einsatz.php` | Die Höhe des Einsatzorts (`site_ele_m`) erscheint nur an einem **luftgebundenen** Diensttag (`m.day_kind === 'air'`). Ein Bergwacht-Einsatz am Boden auf 1200 m führt die Höhe in der Datenbank, zeigt sie aber nicht. | nein | **nicht behoben.** Das ist eine Anzeigeentscheidung aus A13 und nicht Gegenstand dieses Pakets; anders als bei den Fähigkeiten hat der Auftraggeber dazu nichts gesagt. Als Beobachtung hier festgehalten — falls sie ihn stört, gehört sie zu Nr. 198 in dieselbe Runde. |
