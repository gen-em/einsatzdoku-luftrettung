...
> | Paket in Arbeit | **D (Doku)** als nächstes auf `claude/s7-umsetzung-vorbereiten-s8kax0`. **C (Uhr) ist fertig** auf `claude/s5-paket-c-uhr`; E (Android-Zusatz) läuft in eigener Instanz |
> | Erledigt | **C — Uhr** (03.09.2026, Uhr **3.0.0**): `PairView.mc` neu, `Pair.mc` neu geschrieben, Vorgabewert `nadoku.gen-em.org`, Wortliste-Bereich `e`, Tastensperre in beiden Reihenfolgen. Stufe I **99 übersetzt / 0 fehlgeschlagen / 0 Warnungen / 0 Fehler** (gleich dem Ausgangsstand), Stufe II **20 Vertreter / 0 Abstürze**, davon 18 mit `PairView`; Simulator-Rundlauf **5 von 6 Fällen** belegt, dazu eine Bildstrecke über **10 Zustände × 3 Geräte**; Wortliste **0/0/0** in fünf Bereichen (88·30·8·2·35 Dateien), Selbstprobe **21/21**. Freigabe der `PairView` am Simulatorbild erteilt (E-S5-60) — Abschnitt 9 · **A — Server** (03.09.2026, Web **13.0.0**): `pair.php` mit vier Anliegen, `kopplung_lib.php`, Tabelle `pair_sessions`, Migration `2026_09_03_kopplungssitzungen`, drei Töpfe, Obergrenze, `email_maskieren()`, Geräte- und Sitzungsschlüssel als SHA-256, Kopplungsprobe **75/75**, Ingestprobe **24/24**, Geräteprobe **39/39**, Migration auf Bestand (41 = 41) und frisch (übersprungen) gefahren — Abschnitt 9 |
