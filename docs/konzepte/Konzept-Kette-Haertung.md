...
> | In Arbeit | **AP4 abgeschlossen — F3 ist behoben, und der Beleg ist gefahren.** Die Ursache (F-KH-U-25): Die Auslieferungsaktion sendet `RETR` auf ihre Zustandsdatei; fehlt die, steht der Datenkanal per `EPSV` schon, der Server schließt ihn, `basic-ftp` liest `ECONNRESET` auf dem Datensocket. Die Aktion fängt das ab, deutet es als „first publish" und arbeitet mit einem **toten Client** weiter — bemerkt wird es erst beim ersten `MKD`, **drei Schritte hinter der Ursache**. Deshalb stand `ensureDir` acht Trennversuche lang im Verdacht. **Die Abhilfe ist der kleinste Eingriff, den es gibt** (Richtung (e), die das Konzept nicht kannte): `tools/kette/zustand.py` legt die Datei hin, bevor die Aktion läuft — Transport, Aktion und Löschverhalten unverändert. **Der Beleg, Lauf 35545737872:** `🎉 Sync complete`, **688 Dateien, 62 Verzeichnisse, 9,7 MB, 7 Minuten 47, kein `ECONNRESET`** — der **erste vollständige FTPS-Abgleich gegen diesen Server überhaupt** (F-KH-U-28), gefahren gegen ein Probeverzeichnis, **ohne einen Finger an der laufenden Anlage**. Am Ende schreibt die Aktion ihre Zustandsdatei selbst fort; der selbsterhaltende Zustand ist damit durchbrochen. **Zwei Fehlanläufe davor lagen am Werkzeug, nicht am Befund:** die Punktdatei-Falle (`NLST` zeigt sie nicht, F-KH-U-26) und ein deutsches Anführungszeichen in einer Python-Zeile (F-KH-U-27) — beide behoben, beide seither durch eine Prüfung abgedeckt, die es vorher nicht gab (28 Lagen in der Selbstprobe, Syntaxriegel über **48** Werkzeuge in Stufe 1). **Der Schritt ist seit dem Beleg scharf** — er läuft vor jedem Abgleich, auch dem echten, und fasst eine vorhandene Zustandsdatei nie an; die Pfade beider Schritte sind über **drei Fälle gegeneinander nachgerechnet, 0 Abweichungen**. **E-KH-09 ist erfüllt.** Offen: die **Abnahme von AP4** (Probelauf zweimal hintereinander, 0 geplante Löschungen, dann Freigabe) und **Prüfpunkt 20** — das Probeverzeichnis trägt jetzt 9,7 MB und ist rekursiv zu entfernen |
