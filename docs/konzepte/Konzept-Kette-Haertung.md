...
> | | |
> |---|---|
> | Stand | **21.09.2026 — AP1 bis AP6 gebaut; AP2 bis AP6 abgenommen, AP1 haengt am Botschutz. Kette II liegt auf `main` (PR #65, `fb614d1`).** Offen: der Rueckbau von Staging (Pruefpunkt 25c), dann **M1 — blockiert am Botschutz von lima-city**, danach AP7 und AP8. — **AP1 umgesetzt** (Dokumentation; kein Web-Code, keine Versionsstufe). Konzept freigegeben (Auftraggeber, 20.09.2026, ohne Änderungen; Fassung nach der ersten Fortschreibung vom selben Tag: F3 neu gefasst, E-KH-20, AP3/AP4 angepasst, Z1 und Z2 erledigt) |
...
> | Erledigt — AP6 | **GEBAUT UND ABGENOMMEN** (Laeufe 35573954983 und 35574032478, 21.09.2026; F-KH-U-39). Offen ist allein der Rueckbau von Staging, Pruefpunkt 25c -- es steht noch in Wartung und das FTP-Passwort ist noch falsch. Der rote Faden ist einer: *Die Kette soll scheitern koennen.* Zehn von zehn fremden `uses:`-Zeilen auf 40-stelliger SHA (die zwei lokalen koennen keine tragen und sind strenger ohne); zwei `concurrency`-Gruppen ohne Abbruch; die fuenf Ueberspring-Stellen in Stufe 2 brechen jetzt ab; 14 → 17 Schritte, darunter Adressvergleich, Versionspruefung nach dem Abgleich und ein Schlussschritt, der bei Fehlschlag sagt, was gilt; `tor.py` beantwortet `--frage version` und `--frage wartung` (19 → **29** Lagen, zwei davon E-KH-19). **Der Wartungsschalter ist nach hinten gewandert** -- vorher konnte die Anlage zustehen, ohne dass eine Datei ausgeliefert war. **Fund F-KH-U-35:** Das Tor der gruenen Laeufe zaehlte Laeufe statt Auslieferungen. **Was es kostet:** Drei Variablen verlieren ihren Vorgabewert; `FTP_STATE_PFAD` ist auf `staging` gemessen leer, der naechste Lauf dort wird rot -- absichtlich, und er fuehrt damit gleich den Nachweis, den die Abnahme verlangt (Pruefpunkt 24). **Pruefpunkt 25 ist gefahren:** Der provozierte Fehlschlag auf Staging hat den Schlussschritt zweimal ausgeloest, einmal mit und einmal ohne Wartung; die beiden Wortlaute unterscheiden sich an genau der einen Stelle, an der sie es muessen. **Staging ist bis zum Rueckbau (25c) nicht benutzbar** |
> | Erledigt — AP5 | **GEBAUT UND VOLLSTAENDIG ABGENOMMEN.** Produktiv-Haelfte: Lauf 35566000648 (Form und Pflichtfreigabe). Staging-Haelfte: Lauf 35570398032 vom 21.09.2026 -- `staging / ausliefern`, alle **17** Schritte gruen in **49 s**, vorher waren es 4 Schritte in 12 s (F-KH-U-36). Eine Schrittfolge fuer beide Umgebungen: Staging 4 → 14 Schritte (Zielprobe, Backup-Tor, Wartung, Migrationsabfrage, Zustandsdatei), Schutzliste **einmal** mit **acht** Pfaden (E-KH-20 (1) erfuellt), `auslieferung.yml` 1 205 → 657 Zeilen. Umgebungsabhaengig bleiben **zwei** Schritte und **drei** Werte. Verschoben als Text, fuenfzehn zugesicherte Ersetzungen. **Vorher vermessen, nicht geraten.** Das Konzept verlangte die Formwahl „nach Messung"; gemessen wurde in zwei Runden gegen `staging` (35549413032, 35549610955), weil `docs.github.com` vom Läufer dieser Umsetzung aus gesperrt ist und dieses Konzept dreimal bezahlt hat, was eine ungeprüfte Behauptung kostet. **Ergebnis:** Der zusammengesetzte Baustein sieht weder `secrets` noch `vars` und lädt bei dem Versuch gar nicht erst (`Unrecognized named-value: 'secrets'`); der aufgerufene Arbeitslauf kann beides, und `environment:` **bindet** auch als Ausdruck — vier von sieben Werten sind drinnen belegt und draußen leer. **Entschieden: aufgerufener Arbeitslauf** (E-KH-27), **unter Vorbehalt** — ob die **Pflichtfreigabe** mitwandert, ist gegen `staging` nicht messbar und steht als **Prüfpunkt 23**; fragt der erste Probelauf mit der neuen Struktur nicht nach der Freigabe, wird AP5 zurückgenommen. **Bis dahin kein Tag.** **Zwei Befunde fielen nebenbei ab:** die drei FTPS-Zugangswerte liegen zusätzlich eine Ebene über den Umgebungen, womit die Geheimnisprüfung der Kette nicht fehlschlagen kann (F-KH-U-32, E-KH-28, Prüfpunkt 22 — ein Klick der Betreiberin); und Staging fehlt der Zustandsdatei-Schritt aus AP4, läuft also in dasselbe F3, sobald seine Zustandsdatei einmal fehlt (F-KH-U-30) — AP5 behebt es dadurch, dass es AP5 ist. **Z7 ist erfüllt** (Komplett-Backup gelungen; `JOBS_TOKEN` liegt in `staging`, gemessen daran, dass `stufe2` es von dort liest) |
...
**Ein Vorbehalt stand an dieser Entscheidung, und er ist eingelöst.** Die
Messung zeigte, dass die Umgebung **bindet** — nicht, ob die
**Pflichtfreigabe** dem `environment:` dorthin folgt. `staging` hat keine,
also konnte kein Lauf dagegen es zeigen; ein Lauf gegen `produktion` nur zum
Zusehen wäre ein Missbrauch des Tors gewesen. Der Prüfweg war deshalb ein
Probelauf, der ohnehin fällig war (Prüfdokument, Prüfpunkt 23).

> **Gefahren am 21.09.2026, Lauf 35566000648: Der Lauf hat die Freigabe
> angefordert und gestanden, bis die Betreiberin sie erteilt hat.**
> **E-KH-27 gilt damit ohne Vorbehalt; AP5 wird nicht zurückgenommen.**
> Der Ausweichweg — zusammengesetzter Baustein mit durchgereichten
> Eingaben — bleibt gemessen gangbar (F-KH-U-31, Messung B) und wird nicht
> gebraucht. Einzelheiten: F-KH-U-34.
...
#### Umsetzung — Stand 21.09.2026 (2): gebaut

**Der Umbau ist gefahren.** `.github/workflows/ausliefern-lauf.yml` trägt die
Schrittfolge; `auslieferung.yml` ruft sie zweimal auf und reicht `umgebung`
durch. **Verschoben als Text, nicht neu geschrieben** — fünfzehn
Ersetzungen, jede einzeln zugesichert; eine, die nicht genau einmal
getroffen hätte, hätte den Lauf abgebrochen statt still durchzurutschen.

| | vorher | nachher |
|---|---|---|
| Schritte `staging` | 4 | **14** (dieselben wie Produktiv) |
| Schritte `produktion` | 13 | **14** |
| `exclude`-Blöcke | 2 | **1** (14 Zeilen, **8** Pfade) |
| Zeilen `auslieferung.yml` | 1 205 | 657 |
| Umgebungsabhängige Schritte | — | **2** |

**Was Staging dazubekommt:** Zielprobe, Backup-Tor, Wartungsmodus,
Migrationsabfrage — und den Zustandsdatei-Schritt aus AP4 (F-KH-U-30).

**Was die Umgebung noch trennt:** zwei Schritte, beide selbsterklärend (der
Tag-Vergleich; das Tor der grünen Läufe, das auf Staging nach sich selbst
fragen müsste), und drei Werte, die der erste Schritt bestimmt.

**Zwei Riegel sind neu, und beide wehren einen Fehler ab, den erst die
Zusammenführung möglich gemacht hätte:**

1. **Ein `case` statt eines Ausdrucks für die Basisadresse.** Ein
   `A && B || C` fällt auf `C` durch, sobald `B` **leer** ist — nicht nur,
   wenn `A` falsch ist. Mit
   `umgebung == 'produktion' && vars.PRODUKTION_URL || vars.STAGING_URL`
   hätte ein Produktivlauf bei leerer `PRODUKTION_URL` still gegen
   **Staging** gemessen: Backup-Tor an der falschen Anlage, Zielprobe an der
   falschen Adresse, Lauf grün. Der Schritt bricht jetzt ab — auch bei einem
   Umgebungsnamen, den es nicht gibt.
2. **Ein leerer Zielpfad bricht unmittelbar vor dem Abgleich ab**, statt die
   Anwendung in das Wurzelverzeichnis des FTP-Zugangs zu synchronisieren.

**Gemessen nach dem Bau** (nicht gelesen): YAML beider Läufe gültig; **42**
`run`-Blöcke über alle Arbeitsläufe, **0** Syntaxfehler; Kettenaufrufe
**36 Aufrufe, 0 Befunde**; **ein** `exclude`-Block mit **acht** Pfaden über
alle Arbeitsläufe hinweg; alle sieben Verbraucher der drei Werte stehen
hinter dem Schritt, der sie setzt (ausgezählt); Wortliste **0 Treffer**.

**Eine dritte Messrunde war nötig** (Lauf 35560508628) und hat die letzte
Annahme erledigt: `${{ env.X }}` löst in einem `with:`-Block auf. Daran hing
`server-dir` — und ein leeres `server-dir` wäre eine Synchronisierung ins
Wurzelverzeichnis gewesen, im Trockenlauf unsichtbar. Einzelheiten in
F-KH-U-31.

**Nicht mitgemacht, und ausdrücklich so** (Backlog Nr. 260): Zwei
Kommentare in `server/wartung_lib.php` und `server/adminbackup_lib.php`
sagen im Präsens „Ausnahmeliste **beider** FTPS-Schritte". Sie beschreiben
die **ausgelieferte** Kette, und dort stehen die zwei Schritte noch; falsch
werden sie erst, wenn AP5 auf `main` ankommt. Dazu verlangte eine Änderung
...
**Der Riegel unter der ganzen Formwahl ist gefallen** (Prüfpunkt 23,
F-KH-U-34): Lauf 35566000648 hat die Freigabe angefordert und gestanden,
bis sie erteilt war. Dieselbe Runde hat **Prüfpunkt 22** miterledigt — die
drei *Repository secrets* sind gelöscht, und die Zielprobe ist danach mit
den reinen Umgebungswerten durchgelaufen.

**Was der Lauf gezeigt hat:** Jobname `produktion / ausliefern`, `staging`
**übersprungen** (F-KH-U-33 trägt), `stufe2` und `zeiger` ebenso, Zielprobe
grün in 17 s, Abgleich grün in 2,3 s als Trockenlauf, und im Protokoll
`Saving current server state to "/../.deploy-state-produktion.json"` — der
Beleg, dass `env.ZUSTANDSPFAD` im `with:`-Block auch in der echten Kette
auflöst.

**Die STAGING-HÄLFTE ist am 21.09.2026 gefahren — AP5 IST VOLLSTÄNDIG
ABGENOMMEN** (Lauf 35570398032, der Merge von Kette II auf `main`;
F-KH-U-36). `staging / ausliefern` lief **alle 17 Schritte in 49 Sekunden
grün** — Zielprobe zum ersten Mal gegen Staging, Backup-Tor, Wartung an,
Abgleich, Wartung aus. Die Schrittnamen sind dieselben wie auf Produktiv;
der Jobname trägt den Zusatz, wie gemessen.

| | Schritte | Dauer |
|---|---|---|
| `staging` vorher | 4 | 12 s (Lauf 21, 20.09.) |
| `staging / ausliefern` nachher | **17** | **49 s** |

**Was die Abnahme noch nicht abdeckt:** den Lauf mit ausstehender Migration,
der die Wartung anlässt. Dieser Push brachte keine Migration mit — die Frage
kommt also erst beim nächsten Schemawechsel dran, und sie ist dann von
selbst beantwortet. Sie bleibt als Rest notiert, nicht als Prüfpunkt: Eine
Migration eigens zu erfinden, um ein Verhalten zu sehen, hieße die Anlage
für die Messung zu verbiegen.
...
aussehen lässt.** Der rote Faden ist einer: *Die Kette soll scheitern
können.* Fast jeder Punkt beseitigt eine Stelle, an der sie das bisher nicht
konnte.

| | gemessen vorher | gemessen nachher |
|---|---|---|
| Fremde `uses:`-Zeilen mit 40-stelliger SHA | 0 von 10 | **10 von 10** |
| `concurrency` in der Kette | keine | **2 Gruppen**, `cancel-in-progress: false` |
| `exit 0` in `stufe2` (Überspringen) | 5 | **0** |
| Schritte im gemeinsamen Lauf | 14 | **17** |
| `--selbstprobe` von `tor.py` | 19 Lagen | **29 Lagen**, 0 offen |
| Kettenaufrufe | 36 | **38**, 0 Befunde |

**Die zwei lokalen `uses:`-Zeilen sind nicht gepinnt, und das ist richtig**
(Entscheidung des Auftraggebers, 21.09.2026): Ein lokaler Pfad nimmt keinen
Ref und läuft immer auf dem Commit des Aufrufers — strenger als ein Pin. Die
Abnahme sagt „jede `uses:`-Zeile"; die ehrliche Zahl ist **10 von 10
fremden**, nicht 12 von 12. Wer die Zahl 12 sucht, findet sie nicht, und das
soll er auch nicht.

**Reihenfolge (E-KH-06).** Es wanderte genau ein Block, aber der richtige:
„Wartung einschalten" nach hinten, direkt vor den Abgleich. Vorher lagen
`doku`-Kopie, Gesprächslauf-Riegel und Zustandsdatei dahinter — jeder von
ihnen kann scheitern, und scheiterte einer, **stand die Anlage zu, ohne dass
eine einzige Datei ausgeliefert worden wäre.** Wartung an für nichts.

**Schlussschritt (E-KH-06).** `if: failure()`, fragt die Anlage nach
Wartungsmodus und Fassung. Er läuft ohne `set -e`: Er darf nicht daran
scheitern, dass die Anlage nicht antwortet — dann ist genau das die Auskunft,
und sie heißt `unbekannt` und nicht `aus`.

**Zwei neue Tore (E-KH-07).** Der *Adressvergleich* hält `PRODUKTION_URL`
gegen `WACHE_BASIS`: Gehen sie auseinander, liefert die Kette nach A aus und
die Wache bewacht B — **beide Seiten sind für sich grün**, und der
Produktivserver bliebe unbeobachtet. Die *Versionsprüfung nach dem Abgleich*
fragt, ob die Anlage die neue Fassung auch wirklich meldet; der FTPS-Schritt
bestätigt nur, dass übertragen wurde.

**`tor.py` statt einer JSON-Zeile im YAML.** Beide neuen Auskünfte
(`--frage version`, `--frage wartung`) stecken in einer reinen Funktion,
damit die Selbstprobe sie durchrechnen kann. Die zehn neuen Lagen prüfen
nicht die guten Fälle, sondern die halben — und **zwei davon sind E-KH-19**:
Ein Server, der das Feld noch nicht kennt, bekommt `unbekannt` und 1, nie
eine erfundene Null.

**Ein Fund fiel dabei ab (F-KH-U-35):** Das Tor der grünen Läufe zählte
Läufe, nicht Auslieferungen — ein übersprungener `staging`-Job galt ihm als
„stand auf Staging". Seit AP5 ist das noch weniger wert, weil ein Probelauf
`staging` jetzt absichtlich überspringt.

**Was AP6 kostet, und es ist keine Kleinigkeit:** Drei Variablen verlieren
ihren Vorgabewert. Auf `staging` ist `FTP_STATE_PFAD` **gemessen leer** —
der erste Lauf danach wird dort rot. Das ist der Zweck und nicht der Preis:
Ein Vorgabewert, der einspringt, lässt eine falsch eingerichtete Anlage
nicht auffallen. Die Zuarbeit steht als **Prüfpunkt 24**, und sie führt
nebenbei den Nachweis, den die Abnahme ohnehin verlangt („fehlende Variable
→ rot vor jedem Zugriff").

**Erledigt am 21.09.2026.** Die Betreiberin hat eingetragen; fünf Läufe sind
seither durch den ersten Schritt gekommen, der bei jedem leeren Wert
abbricht. Damit sind `FTP_ZIELPFAD` und `FTP_STATE_PFAD` in **beiden**
Umgebungen unmittelbar belegt und `WACHE_BASIS` als Repositoriums-Variable
durch den grünen Adressvergleich (F-KH-U-38). **Die bisherige Ableitung aus
dem Protokolltext ist damit hinfällig** — vorher musste man die Einrichtung
aus dem Verhalten erraten, jetzt sagt sie der Lauf. Das ist der Unterschied,
den der Wegfall der Vorgabewerte gemacht hat.

**GEFAHREN UND BESTANDEN am 21.09.2026 — AP6 IST ABGENOMMEN**
(F-KH-U-39). Der Nachweis des Schlussschritts (**Prüfpunkt 25a und 25b**)
verlangte ein falsches FTP-Passwort in der Umgebung `staging` und, für den
zweiten Teil, eine von Hand eingeschaltete Wartung; die Betreiberin hat
beides gestellt. Zwei Läufe auf `main` (Stand `fb614d1`), beide rot an der
Zielprobe, beide mit gelaufenem Schlussschritt:

| | 25a, Lauf 35574032478 | 25b, Lauf 35573954983 |
|---|---|---|
| **Wartung** | **`aus`** | **`an`** |
...
Der Schlussschritt behauptet nichts, er fragt die Anlage — ein Riegel, der
immer dasselbe meldete, wäre in beiden Läufen ebenso grün gewesen und
hätte im Ernstfall entweder Entwarnung gegeben oder jemanden an einen
Schalter geschickt, an dem nichts zu tun ist.

**Nebenbei unmittelbar gemessen:** `jobs.php` antwortet auch bei
eingeschalteter Wartung — die Fassung `20.26.2` in 25b ist aus einer
zugesperrten Anlage gelesen. Das war bisher nur erschlossen.

**Was der Nachweis nicht umfasst:** Beide Läufe sind **vor** dem
Wartungsschalter gescheitert. Dass der Schlussschritt dasselbe meldete,
wenn die Kette die Wartung selbst eingeschaltet und Schritt 14 dann
abgebrochen wäre, ist erschlossen, nicht gemessen — er fragt in beiden
Fällen dieselbe Anlage mit demselben Aufruf. Warum diese Lage nicht
herstellbar ist, steht oben und in F-KH-U-37, und es ist dort kein Mangel,
sondern der Beleg.
...
**Offen bleibt allein der Rückbau** (Prüfpunkt 25c): Staging steht noch in
Wartung, das FTP-Passwort ist noch falsch. Der Wiederholungslauf danach ist
selbst eine Prüfung — er belegt, dass die Kette nach zwei Fehlschlägen ohne
Handarbeit wieder durchläuft und dass die Zustandsdatei der Aktion (AP4)
die Abbrüche überstanden hat.


### M1 — Erster grüner Produktivlauf (Betreiberin)

> **BLOCKIERT seit dem 21.09.2026, und nicht an dieser Kette** (F-KH-U-36).
> Das Tor der grünen Läufe verlangt einen Lauf, der **als Ganzes**
> erfolgreich war und in dem der Job `staging` gelaufen ist. Der erste Lauf
> mit Kette II auf `main` ist **rot**, weil **Stufe 2** rot ist — der
> Botschutz von lima-city weist die Anmeldung auf `login.php` ab
> (F-KH-U-10, HTTP 403, „Dein Browser wird geprüft").
>
> **Auf keinem Stand, dessen Stufe 2 nie gemessen hat, kommt ein Tag
> durch** — und das ist richtig so, dafür gibt es das Tor. Es heißt aber:
> **M1 wartet auf den Hoster, nicht auf ein Arbeitspaket.** Der Botschutz
> muss für die Läufer ausgesetzt werden (oder für `login.php`).
>
> **Verworfen, bevor es jemand vorschlägt:** den Botschutz im Werkzeug zu
> umgehen. Dann prüfte Stufe 2 gegen eine Anlage, die sich anders verhält
> als die, die Nutzerinnen sehen — eine grüne Zahl ohne Aussage.
>
> **Die Auslieferung selbst ist davon nicht betroffen:** Sie ist gelaufen
> und grün. Stufe 2 misst **nach** dem Deploy.
...
| Provozierter Fehlschlag auf Staging | AP6 | **ERLEDIGT 21.09.2026** — zwei Wortlaute im Prüfdokument (25a/25b), `Wartung: aus` und `Wartung: an` |
