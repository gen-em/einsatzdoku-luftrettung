...
> | In Arbeit | **AP3 — Tor, Zielprobe, Probelauf: gebaut. ACHT Vermutungen zu F3 sind mit je einer Messung ausgeschlossen, und achtmal sagt die Messung dasselbe: `curl` kann, woran die Auslieferungsaktion stirbt.** Ausgeschlossen: Läuferabbild und Node (Z5 Teil 1), Zertifikat (F-KH-U-08), die Bibliothek *als solche* (lima-city grün), TLS-Sitzungswiederverwendung (F-KH-U-16), `MKD`/`CWD` eines neuen Verzeichnisses (F-KH-U-18), Weg zum Datenkanal (F-KH-U-20, viermal sauberes `EPSV`), **Menge und Sitzungslänge** (F-KH-U-22, **80 von 80 in EINER Sitzung**; die Auslieferung legt 62 an) und zuletzt **Abruf, dann Steuerbefehl in derselben Sitzung** (F-KH-U-24, `PWD` beantwortet mit 257). Dafür gebaut und über den Probelauf auslösbar: `--mengenprobe N` und `--sitzungsprobe`. Unterwegs fiel eine tragende Annahme (F-KH-U-23): **`_openDir` listet NICHT**, es sendet `MKD` und `CWD` (`basic-ftp` 6.2.1, Z. 686–689) — der Stacktrace ist eine **Zustandsmeldung**, der Client war beim `MKD` schon tot. Beide Dokumente sind an der falschen Stelle berichtigt, nicht stehengelassen. **Was bleibt:** Kein von außen sichtbares Serververhalten erklärt den Abbruch; der Unterschied liegt in etwas, das die **beiden Clients verschieden tun**, und das ist mit einem zweiten Client nicht mehr auszuleuchten. **Nächster Schritt — Prüfpunkt 18a:** die Aktion selbst reden lassen (`log-level: verbose`), aber gegen ein **Probeverzeichnis** statt den Webroot. Sie misst dasselbe (derselbe Client, echtes `ensureDir`, 688 Dateien) **ohne** Wartungsmodus und ohne halb ausgelieferten Stand. **Auf ausdrückliche Anweisung der Betreiberin am 20.09.2026 gebaut:** die Eingabe `probelauf_gespraech`. Drei Riegel — ohne `probelauf` bricht ein eigener Schritt ab (er läuft **ohne `if`**, weil ein Riegel, der nur greift, wenn die Lage schon stimmt, keiner ist); `server-dir` zeigt auf `.zielprobe-gespraech/`, der **führende Punkt** absichtlich, weil `.htaccess` Punktpfade mit 403 abweist (F-KH-U-13) und dort sonst eine zweite, abrufbare Kopie der Anwendung läge; und `state-name` zeigt auf eine **eigene** Zustandsdatei *innerhalb* des Probeverzeichnisses, weil `../` von dort in den Webroot zeigte. Die vier bedingten Ausdrücke sind gegen **vier Fälle nachgerechnet** (Tag-Lauf ohne `inputs`, Probelauf, Probelauf+Gespräch, Gespräch ohne Probelauf): **0 Abweichungen** — `dry-run` ist die gefährlichste Zeile der Datei. **Das Probeverzeichnis wird danach VON HAND entfernt**; ein Werkzeug, das Verzeichnisbäume auf dem Produktivserver löscht, soll es nicht geben. Selbstproben: Zielprobe **93 Lagen**, Tor **19**, Wache **38**, alle 0 offen. **E-KH-09 steht.** Einzelheiten im Prüfdokument 1.6 und 2 |
