...
> | In Arbeit | **AP3 — Tor, Zielprobe, Probelauf: gebaut. SIEBEN Vermutungen zu F3 sind mit je einer Messung ausgeschlossen — und damit ist der Vorrat erschöpft, den ein zweiter Client prüfen kann.** Ausgeschlossen: Läuferabbild und Node-Fassung (Z5 Teil 1), das FTPS-Zertifikat (F-KH-U-08, behoben), die Bibliothek als solche (lima-city läuft mit derselben Aktion grün), die **TLS-Sitzungswiederverwendung** (F-KH-U-16), das **Anlegen und Auflisten eines Verzeichnisses** (F-KH-U-18, `ensureDir`/`_openDir`), der **Weg zum Datenkanal** (F-KH-U-20, dreimal sauberes `EPSV`) und zuletzt die **Menge und Sitzungslänge**: **80 von 80 Verzeichnissen in EINER FTP-Sitzung, 80 eigene Datenverbindungen, keine Abweisung** (F-KH-U-22, Lauf 35540252565; die Auslieferung legt 62 an). Dafür gebaut: `--mengenprobe N` über die Eingabe `probelauf_mengenprobe`. Der Vorlauf (F-KH-U-21) hatte sich bei 21 von 80 **selbst abgewürgt** — die feste Zeitgrenze von 60 s je `curl`-Aufruf galt auch für den einen Aufruf mit achtzig Zielen; vier Fehler der Probe sind daraufhin behoben (wachsende Zeitgrenze, Abbruch als Befund statt als Absturz mit achtzig Adressen im Protokoll, keine Falschdiagnose „der Server hat abgewiesen", nachgemessene statt gerechneter Restwarnung) und das Aufräumen von 2N Aufrufen auf einen gebündelt — alles im Folgelauf nachgemessen. **Der nächste Schritt ist eine Entscheidung, keine Messung:** entweder die Aktion selbst reden lassen (`log-level: verbose`, **Prüfpunkt 18** — kostet einen ECHTEN Auslieferungslauf, weil sie im Trockenlauf `ensureDir` nie erreicht), oder die Frage an den Hoster, die durch die sieben Messungen beantwortbar geworden ist. Selbstproben: Zielprobe **81 Lagen**, Tor **19**, Wache **38**, alle 0 offen. **E-KH-09 steht: F3 ist so weit eingegrenzt, wie es von außen geht, und weiter nicht benannt.** Einzelheiten im Prüfdokument 1.6 und 2 |
