...
> | In Arbeit | **AP3 — Tor, Zielprobe, Probelauf: gebaut; der Trennversuch ist gefahren.** Vier Läufe, je zweimal: **Die TLS-Sitzungswiederverwendung ist NICHT die Ursache von F3** (F-KH-U-16) — beide Betriebsarten gelingen. **Aber der Versuch maß die falsche Stelle** (F-KH-U-17): Die Probe schrieb in ein *bestehendes* Verzeichnis, die Auslieferung stirbt beim Anlegen eines *neuen* (`ensureDir` → `_openDir`). Die Zielprobe fährt deshalb jetzt **zwei Rundläufe**, der zweite durch ein neu angelegtes Verzeichnis; Selbstprobe **45 Lagen**. **Auch `ensureDir` ist ausgeschlossen** (F-KH-U-18, beide Rundläufe grün). **Offen ist jetzt der Weg zum Datenkanal:** `curl` fällt von `EPSV` auf `PASV` zurück, eine Bibliothek, die das nicht tut, bliebe hängen — die Probe nennt das seit heute in jedem Lauf (F-KH-U-19). **Prüfpunkt 16.** Vorher: **drei Probeläufe gegen Produktiv.** Stand 19:30 UTC: Der Zertifikatsbefund (F-KH-U-08) ist **behoben**, die FTPS-Verbindung kommt zustande, und **die Wiederverwendung der TLS-Sitzung ist gemessen: JA** — damit fällt F-KH-U-09, und **der Trennversuch ist fahrbar**. Offen: der Lauf OHNE Wiederverwendung. Dazu F-KH-U-13: Die Zielprobe hatte sich mit ihrem eigenen Punkt-Präfix ausgesperrt (`.htaccess` → 403) und das als falschen Zielpfad gemeldet — behoben. Vorher: **der erste Probelauf** (Lauf 35531806339, 20.09.2026). Die Mechanik hält: sechs Schritte übersprungen, der Zeiger blieb stehen, kein Byte auf Produktiv. **Die Zielprobe hat sofort einen Befund geliefert — F-KH-U-08: Das FTPS-Zertifikat von Produktiv passt nicht zum Hostnamen**, und die Auslieferungsaktion prüft das nicht. **F3 ist damit NICHT beantwortet** (curl kam nicht bis zum Datenkanal). **PFLICHTSTOPP.** Die Selbstproben sind grün mit Lagenzahl (Tor **17**, Zielprobe **26**); **Z5 Teil 1 ist erledigt und schließt Läuferabbild und Node als F3-Ursache aus** (beide Läufe: 2.337.0 / ubuntu-24.04 20260907.300.1). Der Probelauf gegen Staging und der Trennversuch gegen Produktiv **stehen aus** — sie brauchen die Betreiberin (Prüfpunkte 10 bis 12). Vorher: **AP2 — Zeiger und Wache: gebaut.** Abnahme teilweise: Die Gegenprobe im Werkzeug ist grün (Selbstprobe **38/38**, sechs davon neu zum Vergleichsstand), der Befund B6 ist **ohne Netz aus den Ständen nachgerechnet** — aber der Handlauf gegen Produktiv **konnte nicht laufen** (der Container-Proxy weist `nadoku.gen-em.org` mit `403` ab), und der Zeiger-Job ist **gebaut, nicht gelaufen** (er misst sich erst mit M1). Einzelheiten im Prüfdokument 1.5 |
