...
> | In Arbeit | **AP3 — Tor, Zielprobe, Probelauf: gebaut. Sechs Trennversuche gegen Produktiv, F3 sechsmal nicht gefunden — aber jedes Mal eine Vermutung ausgeschlossen, mit Messung statt Argument.** Ausgeschlossen: Läuferabbild und Node-Fassung (Z5 Teil 1), das FTPS-Zertifikat (F-KH-U-08, behoben), die Bibliothek als solche (lima-city läuft mit derselben Aktion grün), die **TLS-Sitzungswiederverwendung** (F-KH-U-16, vier Läufe), das **Anlegen und Auflisten eines Verzeichnisses** (F-KH-U-18, `ensureDir`/`_openDir`) und der **Weg zum Datenkanal** (F-KH-U-20, zweimal sauberes `EPSV`). Dazu **gebaut und einmal gefahren: die Mengenprobe** (`--mengenprobe N` über die Eingabe `probelauf_mengenprobe`) — sie stellt die EINE lange Sitzung der Auslieferungsaktion nach, die die Zielprobe bauartbedingt nicht messen kann. **Ergebnis des ersten Laufs (F-KH-U-21): 21 Verzeichnisse in EINER Sitzung gingen durch** — die naheliegende Form der Sitzungsvermutung ist damit widerlegt, eine Grenze bei 30 oder 50 bleibt möglich. **Abgebrochen hat nicht der Server, sondern die Probe sich selbst:** Ihre feste Zeitgrenze von 60 s je `curl`-Aufruf galt auch für den einen Aufruf mit achtzig Zielen. Vier Fehler der Probe sind daraufhin behoben — wachsende Zeitgrenze, Abbruch als Befund statt als Absturz mit achtzig Adressen im Protokoll, keine Falschdiagnose „der Server hat abgewiesen", nachgemessene statt gerechneter Restwarnung — und das Aufräumen von 2N Aufrufen auf einen gebündelt. Selbstproben: Zielprobe **81 Lagen**, Tor **19**, Wache **38**, alle 0 offen. **Prüfpunkt 17 ist zu wiederholen; dort hängt AP3.** **E-KH-09 steht: F3 ist eingegrenzt, nicht benannt.** Einzelheiten im Prüfdokument 1.6 und 2 |
