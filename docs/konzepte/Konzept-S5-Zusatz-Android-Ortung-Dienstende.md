...
> | Erledigt | Freigabe und alle sechs Fragen (E-S5Z-15 bis -21); Prüfstand aufgebaut, Ausgangszahlen gemessen; **E1 (Android 0.8.0)**; **Bilderlauf beider Module (0.8.1)**; **E2 (Android 0.9.0)**; **E3 (Android 0.10.0)**; **Stufe II im Emulator und der Fehler, den sie fand (0.10.2)** |
> | Wo es hakt | Nichts Blockierendes. Die Diagnose 1.3 ist **beantwortet** (Abschnitt 8): belegt ist H2. **Der Emulator läuft doch** (03.09.2026, B-S5Z-18): nicht API 34 — dort erschlägt der Watchdog den `system_server` —, wohl aber API 28 `default` mit `-accel off`, Boot 232 s. Damit sind Kopplung, Dienstansicht und der Zustand `FREIGABE_FEHLT` am laufenden Programm belegt, und der Emulator hat sofort einen Fehler gefunden (B-S5Z-17, behoben mit 0.10.2). Ursprünglich stand hier: Die Diagnose kann die Umsetzung nicht fahren — sie braucht das S24 und den Produktivserver. Sie liegt beim Auftraggeber und ist Voraussetzung für den *Beleg* in E2, nicht für den *Bau* (E2 behebt alle drei Ketten unabhängig davon). Dazu: kein Emulator im Container (Vorbereitung 8.3), also ist Paket E gerätegebundener als Abschnitt 9.3 annimmt |
...
| **B-S5Z-17** | **Die Dienstansicht merkte eine anderswo erteilte Ortungsfreigabe nicht.** `var ortungFrei by remember { mutableStateOf(checkSelfPermission(…)) }` wertet einmal aus; fortgeschrieben wurde der Wert nur im Rückruf der Berechtigungsfrage, einen `onResume`-Beobachter gibt es im Handy-Modul nicht. Wer einmal „Nicht mehr fragen“ gewählt hat, kann die Freigabe **nur** in den Android-Einstellungen erteilen — und las danach weiter „Ortung nicht freigegeben“. Erst ein vollständiger Neustart räumte es weg. **Gefunden im Emulator, nicht durch Lesen:** Abzug nach `Home` und Rückkehr byteweise gleich dem davor (md5 `08c9a62a…` zweimal), nach `force-stop` und Neustart verschieden. Die Zeile stammt aus Android 0.3.0, ist also älter als Paket E — trifft aber die Zusage von E1 | `HauptActivity.kt` | **Behoben** mit 0.10.2: Die Freigabe hängt am vorhandenen Sekundentakt, mit dessen eigener Begründung („DER ZUSTAND WIRD ABGEFRAGT, NICHT GEHALTEN“). Gegenprobe: dieselben zwei Abzüge jetzt verschieden (`5819f85b…` gegen `ec26ad4f…`), ohne Neustart |
| **B-S5Z-18** | **„Kein Emulator im Container“ war falsch, und der Satz kostete Paket E die ganze Stufe II.** KVM fehlt wirklich (`/dev/kvm` weg, weder `vmx` noch `svm`, `-accel-check` sagt es). Daraus folgt aber nicht, dass kein Emulator läuft: Mit `-accel off` übersetzt QEMU selbst. Der Satz verwechselte „startet nicht ohne Weiteres“ mit „geht nicht“ und verhinderte den Startversuch. Drei Anläufe: API 34 `google_apis`/3 GB — kein Bootabschluss nach 23 min, Speicher 2,82 von 3,05 GB, CPU 276 von 400 % untätig; API 34 `default`/6 GB — Watchdog-Schleife, `systemReady()` über 60 s, `*** GOODBYE!`; **API 28 `default`/4 GB — Boot 232 s, 0 Abbrüche** | `android/LIESMICH.md`, `CLAUDE.md` 6 | **Behoben**: Regel in `CLAUDE.md` 6 (Emulator ist Pflicht bei Android-Arbeit), Werkzeug `android/werkzeuge/emulator.sh`, Zahlen und Stolpersteine in `LIESMICH.md`. **Rest:** Unter TCG läuft die *Systemoberfläche* zeitweise in den ANR; `bild` prüft deshalb `mCurrentFocus`, bevor es abzieht |
