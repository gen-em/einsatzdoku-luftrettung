...
| **134** Neufassung (vierte) | `tools/ingestprobe/` Teil 9, Fälle 1, 4, 8a, 8b und 10; eigenes Messgeschirr an vier Fällen; Gegenprobe gegen den Stand davor auf eigenem Port | Zeiten, die nicht zum Tag passen, werden abgewiesen; ein **nachgelieferter** Dienst bekommt Beginn und Ende | **59 Erwartungen, 0 nicht erfüllt** (vorher 56); am Stand vor der Neufassung **59/4** — Fall 1 und 8a antworten dort `200` statt `400`, und der nachgelieferte Dienst bekommt `ended_at` `null` statt der gesendeten Zeit (beide Erwartungen von Fall 10). Messgeschirr `np_regel.php` an vier Fällen (Nachlieferung, Angriff, neuer Datensatz am alten Tag, zweites Paket): **12 Erwartungen, 0 nicht erfüllt**; am Stand davor **12/8** |
...
| `tools/ingestprobe/` | **59 Erwartungen, 0 nicht erfüllt** | vorher 39, 47, 53, 54, 56; Teil 9 neu, Zeitstempel auf `time()` umgestellt; sechs Erwartungen zur Nachbesserung Nr. 134, eine zur zweiten Runde (vorgehende Uhr), zwei zur Wiederaufnahme (Tageszeitraum, Papierkorb im offenen Fenster), drei zur Neufassung (abgewiesener Widerspruch, plausible Zeiten am alten Tag, nachgelieferter Dienst) |
...
| **eigener Fund** | **Die dritte Nachbesserung zu 134 brach den nachgelieferten Dienst**: Ein Dienst, der später als 72 h nach seinem Datum hochgeladen wird, bekam keinen Tageszeitraum mehr — die Bremse fragte den Tag nach *seinen* (vom Absender gesendeten) Zeiten. Dazu die eigentliche Ursache, die bis dahin offen war: Der **Einsatz selbst** stand mit 96 Jahren Dauer in der Datenbank, geschützt war nur der Tag | mittel / mittel | **behoben** `d4eb0a3` — `pruef_zeit_zum_tag()` in der gemeinsamen Prüfschicht weist ein Paket ab, dessen Zeiten nicht zu seinem `day` gehören (`400`); die Bremse fragt jetzt nach dem jüngsten `created_at` der übrigen Datensätze des Tages. Probe 56 → **59/0**, am Stand davor 59/4; Messgeschirr 12/0 gegen 12/8. Gefunden beim Nachfahren des Skeptiker-Skripts gegen den eigenen Stand — nicht von einem Angreifer gemeldet |
