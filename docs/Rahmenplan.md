...
**Fassung 78 (17.09.2026)** — dieses Dokument steuert das Programm:
...
**Stand am 17.09.2026, gemessen an `origin/main` (`14f99ac`):** **Web
20.15.2**, **Uhr 3.1.0**, **Android 0.15.0** — unverändert gegenüber
Fassung 77, weil seither nichts auf `main` gekommen ist. **Schritt 10a (P5a)
ist gemergt** (PR #50, `14f99ac`, 16.09.2026 — Web 20.4.0 bis 20.15.2, Uhr
und Android unberührt). **`update.php` ist fällig** — P5a bringt sieben
Migrationen mit (`csp_berichte`, `job_laeufe`, `mail_warteschlange`,
`ratenschutz_stufen`, `sicherheit_ereignisse`, `geraet_abgewiesen`,
`sicherungsziel_aufbewahrung`).

**Schritt 10b (P5b) läuft auf einem Zweig und ist NICHT auf `main`:**
`claude/magical-dirac-we2y1z`, **Web 20.15.3 bis 20.21.1**, sechs der zehn
Arbeitspakete gebaut, **fünf weitere Migrationen**. Wer die Zahlen oben für
den Stand des Programms hält, unterzählt um **acht** Auslieferungen
(20.15.3, 20.16.0, 20.17.0, 20.18.0, 20.19.0, 20.20.0, 20.21.0, 20.21.1 —
nachgezählt an `server/version.php` je Commit) — sie
messen `main`, und das ist Absicht (Regel aus Fassung 42). Einzelheiten in
Fahrplanzeile 10b und im Schritt-10-Block.

**Der Kopf dieses Dokuments stand bis hierher auf Fassung 74, während
...
(Abschnitt 9, Regel aus Fassung 42) — dieselbe Regel, auf die sich der
Absatz weiter unten schon zweimal beruft. **Wer eine Zeile in Abschnitt 10
...
**Schritt 9d (Demo-Ausbau) ist gemergt** (PR #48, `7f334cb`, 15.09.2026 — Web 20.3.0, Uhr und Android
...
| 10b | **P5b — Konto und Registrierung** (R82) | Registrierung mit drei Betriebsarten und Sicherheitspaket; Konto-Lebenszyklus (Bibliothek, Kontostatus bis `ingest.php`, Double-Opt-In, Selbstlöschung mit Karenz, E-Mail-Wechsel, Einwilligungen mit Fassungskennung); Onboarding mit Notfallblatt; Geräteschlüssel auf SHA-256; Mengengrenze je Konto; Aufbewahrung je Konto (Nr. 48); Demo-Konto je Betriebsart (R25) | Schritt 10a **erfüllt** (am 16.09.2026 auf `main` gemergt, PR #50, `14f99ac`); **Schritt 15** war als zweite Voraussetzung der Umsetzung gesetzt und **ist offen** — die Umsetzung läuft seit dem 16.09.2026 trotzdem. Von Schritt 15 hat AP2 allein den **Token-Teil von Paket 1** (Nr. 202) mit erledigt; die übrigen Pakete stehen, die Abweichung von der Reihenfolge 10a → 15 → 10b bleibt also bestehen und ist in der Statusspalte begründet. Das Konzept durfte ohnehin parallel entstehen | **liegt vor und ist freigegeben** (Fable, 16.09.2026; Freigabe des Auftraggebers am selben Tag, **ohne Änderungen**): `docs/konzepte/Konzept-P5b-Konto-und-Registrierung.md`, **E-P5b-01 bis -24**, **AP1–AP10**, zwei Fable-Schritte (M-P5b-01 Dokumentseite, M-P5b-02 Registrierung/Erststart/Rückfrage/Notfallblatt). **Legt den Protokoll-Schreibweg als erstes Paket fest** (V1 entschieden 16.09.2026: Betriebsereignisse, keine Zugriffe). Prüfdokument daneben (`Pruefdokument-P5b-Konto-und-Registrierung.md`); Mockups in `docs/konzepte/konzept-p5b/mockups/` — **fünf Darstellungen** (M-P5b-01, -02a bis -02d), vier davon zusätzlich bei 376 px: **9 HTML, 9 PNG** und `LIESMICH.md` | Fable (Konzept), Umsetzung Opus; **die beiden Mockup-Schritte ebenfalls Fable.** Hier stand bis Fassung 78 „mit Opus statt Fable" — das galt für zwei Entwürfe, die dieser Zweig am 16.09.2026 auf Weisung selbst gebaut hat. Der Auftraggeber hat am 17.09.2026 die Fable-Fassung aus der Konzeptsitzung nachgereicht; die Opus-Entwürfe sind abgelöst und entfernt | **In Umsetzung** auf `claude/magical-dirac-we2y1z` seit dem 16.09.2026 — **sechs der zehn Pakete sind erledigt**, in fünf Commits: **AP1** Protokoll-Schreibweg mit der Karte „Konten" und der Zählkarte (Web 20.16.0, `b9046ed`; **Nr. 216** nebenbei behoben), **AP2** Lebenszyklus-Bibliothek (20.17.0, `442dbcb`) — damit ist der **Token-Teil von Nr. 202 Paket 1** erledigt, und zwar hier und nicht in Schritt 15, weil die Selbstregistrierung aus AP3 sonst die **fünfte** Fassung derselben Tokenanlage geworden wäre (begründet in `server/version.php` bei 20.17.0) —, **AP7** Demo-Anmeldung abschaltbar (20.18.0, `ad93c9e`), **AP4** Einwilligungen (20.19.0) und **AP5** Selbstlöschung mit Karenz samt Adresswechsel (20.20.0) zusammen in **einem** Commit (`edc3040`), **AP6** Mengengrenze und Aufbewahrung je Konto (20.21.0, `533850d`; **Nr. 48** erledigt, von **Nr. 37** nur der Speichergrenzen-Teil — der Rest dieses Eintrags, vor allem `post_max_size` der Zielanlage, ist unberührt). Dazu **Web 20.21.1** (`de82dea`) mit drei Funden außerhalb der Pakete (**Nr. 217, 218, 219**); den ersten hat **kein Prüfmittel gemeldet** — die Profilseite lag zehn Tage lang außerhalb ihres Gerüsts, während der Bilderlauf für sie drei Nullen meldete, gefunden wurde sie beim **Ansehen** eines Bildes. Die Gegenprobe zählt seither je Seite die Karten außerhalb von `main.inhalt`: **53 Seiten, 149 Karten, 0 außerhalb**, mit wieder eingebautem Fehler **6 geprüft, 4 außerhalb**. **Beide Mockups sind am 17.09.2026 geliefert und freigegeben** (M-P5b-01, -02a, -02c, -02d als V1; **-02b als V1.1**, nachdem der Auftraggeber Aktionen und Plaketten rechtsbündig und alles vertikal zentriert verlangt hat). **AP3, AP8 und AP9 sind damit ohne weitere Pause umsetzbar** — berichtigt mit Fassung 78: hier stand „AP3, AP8 und AP9 warten darauf", und das gilt seit dem 17.09.2026 nicht mehr. Die **vier Gestaltungsvorgaben** vom 17.09.2026 (Konzept Abschnitt 6) gelten für die **Umsetzung**, nicht nur für die Mockups. **Offen: AP3, AP8, AP9 und der Abschluss AP10.** Auf dem Zweig liegen **fünf Migrationen** (`2026_09_16_protokoll_ereignisse`, `…_konto_lebenszyklus`, `…_einwilligungen`, `…_adresswechsel_bestaetigt`, `…_konto_grenzen`) — **`update.php` ist nach dem Merge fällig**. **Zuarbeit bleibt:** anwaltliche Prüfung der drei Entwürfe unter `docs/rechtstexte/` (`AVV.md`, `Nutzungsbedingungen.md`, `Datenschutz-Ergaenzung-P5.md`; E-P5b-24) vor dem Einspielen in `rechtstexte`; AP4 baut so lange mit Platzhaltern. **Code auf dem Zweig, nicht auf `main`** |
...
**Sechs der zehn Pakete sind gebaut**, auf `claude/magical-dirac-we2y1z`,
**Web 20.15.3 bis 20.21.1**, 16. und 17.09.2026: AP1 Protokoll-Schreibweg
(20.16.0) · AP2 Lebenszyklus-Bibliothek (20.17.0, Backlog Nr. 202 Paket 1
...
Aufbewahrung je Konto (20.21.0; **Nr. 48** erledigt, von **Nr. 37** nur der Speichergrenzen-Teil). **Offen sind AP3**
(Registrierung), **AP8** (Handbuch und „Was ist NAdoku"), **AP9**
(Onboarding und Rückfragen) und **AP10** (Abschluss). Daneben stehen
**fünf Funde außerhalb der Paketaufträge, Nr. 215 bis 219**: Nr. 215 —
...
eigene 20.15.2 hatte und der Merge die Kollision auflösen musste; Nr. 216
(`frame-ancestors` in einer Report-Only-Richtlinie, also wirkungslos)
kam mit AP1 in 20.16.0; Nr. 217 (Profilseite aus dem Seitengerüst
ausgebrochen) und Nr. 218 (Meldungston ohne Regel) in 20.21.1. **Nr. 219
...
nicht nebenbei getroffen. Dabei sind **fünf Migrationen** entstanden: die
Tabellen `protokoll_ereignisse` (AP1) und `konto_einwilligungen` (AP4,
dazu `rechtstexte.stand_am` von DATE auf DATETIME), dazu dreimal `users`
erweitert — Lebenszyklus (AP2, fünf Spalten und ein Index), Adresswechsel
(AP5), Mengengrenzen (AP6). **`update.php` ist nach dem Merge fällig.**
...
| 48 | Aufbewahrung je Konto | **erledigt P5b/AP6** (17.09.2026, Web 20.21.0) | `users.backup_pakete` schlägt die Zahl der Installation (`adminbackup_aufbewahrung`, Vorgabe 2); `NULL` heißt „die Vorgabe gilt", `edbak_aufbewahrung_konto()` liest den Wert über die Kontokennung, `edbak_verdraengen()` benutzt ihn. Abnahme 4 Fälle, **4 bestanden**. **Die Kurzfassung ist die Falle gewesen:** „Aufbewahrung je Konto" steht so auch über AP6 im Konzept, und AP6 hat es als Aufbewahrungs**frist für Einsätze** gelesen und ein stillschweigend löschendes Feld gebaut — berichtigt vor dem Commit, also nie ausgeliefert (F5 im Prüfdokument). Eine Backlog-Nummer ist ein **Verweis, kein Titel**; wer sie umsetzt, liest den Eintrag |
...
| **Nach dem Merge von P5b: `update.php` aufrufen — FÜNF Migrationen** (Stand AP6, Web 20.21.1): `2026_09_16_protokoll_ereignisse` (AP1), `…_konto_lebenszyklus` (AP2), `…_einwilligungen` (AP4), `…_adresswechsel_bestaetigt` (AP5), `…_konto_grenzen` (AP6) — ein Aufruf verbucht alle fünf. **AP3, AP8 und AP9 stehen noch aus und können weitere bringen**; maßgeblich ist die Liste unter Betrieb → Updates, nicht diese Zeile. **Still bleibt es diesmal nicht:** Seit P5a/AP3 schaltet der Torwächter in `auth_guard.php` bei ausstehender Migration den **Wartungsmodus** ein, sobald sich die erste angemeldete Anfrage meldet — Betrieb → Updates bleibt als Ausnahmeseite erreichbar, alles andere ist zu, bis „Ausstehende ausführen" gedrückt ist. Eine Ausnahme ist das Einwilligungstor: Fehlt `konto_einwilligungen`, lässt es ausdrücklich jeden durch, statt alle auszusperren (`einwilligung_lib.php`) | Schritt 10b (AP1, AP2, AP4, AP5, AP6), `CLAUDE.md` 3 | **nach dem Merge**, vor der ersten Anmeldung |
| **Anwaltliche Prüfung der drei Rechtstext-Entwürfe** (E-P5b-24) — `docs/rechtstexte/Nutzungsbedingungen.md`, `AVV.md` (übernimmt die EU-Standardvertragsklauseln nach Durchführungsbeschluss (EU) 2021/915 unverändert durch Verweis und füllt die Anlagen I–IV aus) und `Datenschutz-Ergaenzung-P5.md` (elf Bausteine B1–B11 zum Einarbeiten in die bestehende Erklärung). Darin stecken die Festlegungen vom 16.09.2026: Gen-EM GbR als Vertragspartner, unentgeltlich ohne Verfügbarkeitszusage, Haftung nur für Vorsatz und grobe Fahrlässigkeit mit den gesetzlichen Ausnahmen, Nutzerkreis ärztlich im Rettungsdienst mit Zusicherung der Berechtigung, deutsches Recht, Subauftragsverarbeiter dataforest (Hosting) und lima-city (Mail). **Mitprüfen lassen: die 30-Tage-Frist für IP-Adressen** (E-P5b-06) — die P5c-Vorbereitung verlangt dafür eine juristische, keine technische Bestätigung. **Geschrieben sind die Texte, geprüft nicht** — deshalb hat AP4 (Web 20.19.0) die Mechanik gebaut und die Texte weggelassen: Ein Text ohne `stand_am` gilt als „nicht in Kraft" (`einwilligung_lib.php`, Zeile 98), das Tor in `auth_guard.php` steht still. **Das Einspielen ist der Schalter:** Mit der ersten Fassung landet jedes bestehende Konto beim nächsten Login am Tor — Nutzungsbedingungen und AVV sperren, die Datenschutzerklärung zeigt nur einen Hinweis (E-P5b-05) | Schritt 10b (AP4), R41 | **vor dem Einspielen in `rechtstexte`** — spätestens mit dem Umschalten der Registrierung auf *offen* (E-P5b-01), vor Welle 1. Mit **S11** eine zweite Runde: Dort verschiebt sich die Grenze der Verschlüsselung, und alle drei Texte ändern sich |
...
| R9 | Registrierung in drei Betriebsarten plus Sicherheitspaket | gilt, P5 (konkretisiert in R37). **Vorgabe und Ort seit 10b entschieden** (E-P5b-01): *offen* · *offen mit Freischaltung* · *nur auf Einladung*, Vorgabe **nur auf Einladung** — die sicherste Grundstellung für eine Selbsthosterin, die diese Seite nie aufschlägt, und nicht die, die nadoku fahren wird. Die Einstellung steht seit **Web 20.16.0** (10b AP1) in Betrieb → Servereinstellungen, Karte „Konten" (E-P5b-14), **hat aber noch keinen Verbraucher**: Die Registrierungsseite (E-P5b-13) kommt mit 10b AP3. Die Öffnung in Wellen (R41) fährt über genau diese drei Werte |
