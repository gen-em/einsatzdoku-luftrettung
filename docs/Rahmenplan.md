...
**Fassung 49 (13.09.2026)** — dieses Dokument steuert das Programm:
...
(179) und der Sollwert, den es seit Web 15.5.0 nicht mehr gibt (180).
...
| 41 | **Zwei** Klassen ohne Regel (von fünf) | **Mockup-Runde** (9c) | **Drei der fünf sind mit Backlog-Runde 3 gestrichen** (Web 19.3.1, 13.09.2026): `rea-kopf`, `rea-beginn` und `phasen-name` sind aus dem Markup und stehen mit Begründung auf `streichliste.md`; `pruefen.py` meldet seither **2** statt 5 `[offen]`. Offen bleiben `imp-warn` und `imp-daygroup` — sie brauchen je eine Regel und damit ein Mockup. **Entschieden 12.09.2026:** `imp-warn` und `imp-daygroup` bekommen eine Regel (zwei neue Darstellungen → Mockup), `rea-kopf`, `rea-beginn` und `phasen-name` werden mit Begründung gestrichen; `rmneu` war seit S9/AP1 beantwortet — fünf, nicht sechs |
| 42 | Drei Unicode-Symbole im Markup | **Mockup-Runde** (9c) | **Entschieden 12.09.2026: beide ersetzen** — `×` am Koordinaten-Chip (`.rmx` neu bemaßen) und `⚠` im Fließtext; der `✕`-Rückfall in `wegKnopf()` bleibt als begründete Ausnahme |
...
| 45 | Dritte Kartengröße | **Mockup-Runde** (9c) | **Entschieden 12.09.2026: bauen, Mockup zuerst** — verlässt „nach v1.0" (dort noch acht) |
...
| 178 | Die Kopplungsprobe wirft jede Meldung „Failed to load resource" weg | Backlog-Runde | Fund 13.09.2026 beim Beheben von Nr. 176, beim Durchsuchen der Geschwisterwerkzeuge. `tools/kopplungsprobe/rundlauf.mjs` filtert nur über den Text und verwirft mit der dritten Alternative jede Ressourcenmeldung — gemessen **4 von 8** gebauten Fällen falsch, darunter ein **404** und ein **500** auf der eigenen Basis, für die `requestfailed` nicht feuert. Die beiden anderen Browserwerkzeuge sind in Ordnung (geprüft) |
| 179 | Die Zusage „keine fremde Quelle zur Laufzeit" zählt kein Prüfmittel nach | Backlog-Runde, zuarbeitend zu P6 (R69) | Fund 13.09.2026 aus der Gegenprüfung des Nr.-176-Nachtrags: `tools/vollstaendigkeit/` Gruppe 5 kennt **zwei** Zusagen, sieht keine Adresse an, und die Anwendung schickt **keine** CSP (0 Fundstellen unter `server/`). Der Kommentar zu Nr. 176 hatte die Lücke an dieses Mittel weitergeschoben — derselbe Fehler wie 176, nur umgekehrt; berichtigt. Der Inhalt der Aufgabe ist die **Ausnahmeliste**: Kacheln und Ortssuche sind gewollte Laufzeitquellen |
| 180 | Die Kopplungsprobe misst gegen einen Sollwert, den es seit Web 15.5.0 nicht mehr gibt | Backlog-Runde | Fund 13.09.2026 beim Nachfahren der Probe für Nr. 178: `rundlauf.mjs:214` verlangt **44 px** für jeden Knopf, seit Web 15.5.0 gelten aber zwei Sollwerte (E-S8-09/R76) und bei 1280 px am Zeigergerät sind **36 px** richtig. Gemessen **25 Erwartungen, 1 nicht erfüllt**; der Fehler liegt im Mittel, nicht in der Anwendung (Bilderlauf: 0 von 360 falscher Höhe). **Rot seit dem 06.09.2026**, gefahren erst heute |
...
und die dritte ist die unangenehmste: Die Kopplungsprobe verlangt 44 px für
jeden Knopf, seit Web 15.5.0 gelten zwei Sollwerte, und sie ist deshalb **seit
dem 06.09.2026 rot**. Gefahren wurde sie erst heute, weil ein anderer Punkt
dazu führte. Ein Prüfmittel, das niemand fährt, ist kein Prüfmittel.
