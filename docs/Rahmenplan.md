...
**Fassung 121 (25.09.2026)** — dieses Dokument steuert das Programm:
...
nennen. **Die Reihenfolge der offenen Schritte ist:** **10c** (läuft seit
23.09.2026) → **SD** → **17** → **18** → 12 → 12a → 13 → 14; **PK-06 bis PK-08**
...
| 80 | Auswertung der Gerätestatistik (Rest von 59) | **P5a** (Nachlöse-Job, E-PP-06) / **10c AP7** (Herkunft und Dashboard; die Vorbedingung gilt für die Summenzählung nicht, die User-Agent-Hälfte ist gestrichen — E-P5c-45) | Gerätemodelle und Nutzung sind **gebaut** (S8 AP4, Betrieb → Statistik). Offen: **Herkunft je Einsatz (R64-Werte) und Betriebslage-Dashboard**, mit der Datenschutzerklärung als Vorbedingung (Abschnitt 6) |
...
| 121 | Vorschau der Rechtstexte beim Tippen (Mockup 09) | **10c AP9 — gebaut** (Web 21.1.0, auf dem Zweig) | heute nur der gespeicherte Stand — Umverteilt 20.09.2026 (E-P5c-20) — die Rechtstextseiten werden in 10c ohnehin angefasst. |
...
| 141 | Zweitfaktor für alle Konten (K-5) | **10c AP5 — gebaut** (Web 20.42.0, auf dem Zweig) | erweitert R38; QR-Code, zehn Codes, „Gerät merken" mit Nr. 242 in Schritt 18 (E-P5c-41); der Rückweg über den Wiederherstellungsschlüssel mit RW (E-P5c-104) |
...
| 168 | **Zentrale Stammdaten vollständig zurückbauen** (R39) | **10c AP8 — gebaut** (Web 21.0.0, auf dem Zweig) | S9 hat nur die Tür geschlossen (Weg c, kein Schema). Die Fundliste — **208 Befunde**, davon 23 am Schema — ist mit AP8 gelöscht, Historie in Git. Die Migration zählt vor dem `ALTER TABLE` die Zeilen mit `user_id IS NULL` und sperrt ohne Freigabe (E-P5c-48, -125) |
...
| 169 | Ein Diensttag mit „Anderem Rettungsmittel" kann keine Besatzung festhalten | **10c AP8 — gebaut** (Web 21.0.0, auf dem Zweig) | Weg (b) (E-P5c-47): die Rollen der gewählten Betriebsart; eine Migration trägt sie für den Bestand nach (E-P5c-123). Bis dahin galt (c) seit dem 12.09.2026 |
...
| 190 | Statistikseite lässt das virtuelle Gerät stehen, „Ohne Gerät" zählt zu niedrig | **10c AP7** (Beifang, E-P5c-46) | Fund 14.09.2026 (Bestandsaufnahme R42): Die Kontenabfrage hat **gar keine** `manual-%`-Bedingung. Das virtuelle Gerät entsteht an vier Stellen (Handeintrag, CSV-Import, Schnitt, GPX-Import) als echte `devices`-Zeile — wer nur von Hand dokumentiert, fällt aus genau der Gruppe heraus, die die Kleinzeile „sie tragen von Hand nach" meint. `GERAETE_ECHT_SQL` steht in `db.php`; fünf Abfragen benutzen sie, drei schreiben das `LIKE` von Hand |
| 191 | Der von R38 bestellte Index auf `missions(started_at)` fehlt | **10c AP7** | Fund 14.09.2026: nie gelegt (`schema.sql` führt nur `uq_dev_ref`, `idx_user_started`, `idx_day`). Die S8-Seite zählt nach Diensttag und braucht ihn nicht; das Dashboard zählt nach `started_at` und braucht ihn — die einzige Schemaarbeit des Minimalumfangs. Hängt an Nr. 192: nötig nur, wenn dort `started_at` gewinnt |
| 192 | R38 und die S8-Statistikseite zählen Verschiedenes | **erledigt sich mit 10c AP7** — eine Zählung ab Beginn des Einsatzes (E-P5c-18, 20.09.2026) | Fund 14.09.2026, drei Abweichungen: „aktiv" als ODER gegen zwei getrennte Zeilen · 7/30/180 Tage gegen 24 h/7 T/30 T (die Fenster stehen schon als **Nr. 122**) · Zählung nach `days.day` gegen `started_at`. Die Seite setzt R38 nicht um — sie ist der vorgezogene Teil von Nr. 80; entsteht das Dashboard, stünden zwei Zählweisen nebeneinander |
...
| 211 | `/api/`-Aufruf ohne Sitzung | ~~Schritt 17~~ **erledigt in 10c AP5** (Web 20.42.0, auf dem Zweig) | Einschub 20.09.2026; mit dem Code-Schritt wurde der Randfall zum Normalfall (E-P5c-106) |
...
| 244 | Einstellungen-Übersicht **und vor allem das linke Menü** (`ui_leiste_einstellungen()`) | **10c AP9 — gebaut** (Web 21.1.0, auf dem Zweig) | Einschub 20.09.2026, berichtigt 20.09.2026 (M-P5c-01, E-P5c-29): Gemeint war vor allem die **Leiste**. Entschieden: Leiste mit Bereichsüberschriften **Option 1 „Linie"**, Übersicht mit **Bereichskarten**. **Der Fable-Schritt ist erledigt** — kein weiteres Mockup |
| 245 | Erklärtext-Regel und Überarbeitung aller Texte in Verwaltung und Betrieb | **10c AP9 — gebaut** (Web 21.1.0, auf dem Zweig) | Einschub 20.09.2026; Regel kommt in `docs/Design.md` — geschrieben von 10c, nicht hier |
| 246 | Schlüsselblatt und Notfallblatt: Druckseite | **10c AP9 — gebaut** (Web 21.1.0, auf dem Zweig) | Einschub 20.09.2026; ändert E-P5b-09. Berichtigt 20.09.2026 (E-P5c-08/-30): oben **Bildmarke + Kurzname der Installation** (`instanz_kurz()`) statt eines festen „NAdoku" — das wäre auf einer umbenannten Installation falsch; **Webversion in der Fußzeile**. Freigegeben am Bild **M-P5c-01f** |
...
| 253 | Datum-Zeit-Trenner vereinheitlichen | **10c AP2 und AP9 — gebaut** (Web 21.1.0, auf dem Zweig) | Schritt 15 (F-ZE-3); entschieden: das **Komma** (E-P5c-37); gemeint war ` · `, kein Gedankenstrich. Nachgetragen mit Fassung 108 — die Zeile fehlte (F-P5c-50) |
...
| 269 | `assets/schluesselblatt.js` — bei Netzausfall eine stille Sackgasse | **10c AP9 — gebaut** (Web 21.1.0, auf dem Zweig) | Schritt 15; Beifang (E-P5c-46) |
