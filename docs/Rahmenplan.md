...
| 155 | Die Fixture des Referenzbestands trägt die alte Rundenzahl | Backlog-Runde | **teilweise erledigt** (Web 19.1.2): die zwei Riegel stehen, der Neubau der Fixture nicht. Offen sind (a) Demo-Konto anheben und Fixture neu erzeugen und (b) **320 000 aus `KDF_ITER_LISTE` streichen — eigenes Paket, eigene Version**: Fehlt der Wert in der Liste, rechnet der Browser das Token dafür nicht mehr, und jedes Konto auf 320 000 kommt nicht mehr herein — rückholbar nur per Code-Änderung und Deploy, nicht in der Verwaltung |
