...
> **Stand am 06.09.2026:** Schritt 6 ist gemergt (PR #33); `main` trägt
> **Web 14.2.2**, **Uhr 3.0.2** (PR #34) und **Android 0.13.0**. Der Zweig
> `claude/android-ui-bugs-uc1gcv` trägt **Android 0.13.1** — vier Befunde
> vom ersten Blick auf das S24, nur `android/` und Doku, kein Deploy. Dabei
> ist Backlog 81 am Code gefunden worden (fester Vordergrund im adaptiven
> Symbol); Backlog **117** (Symbol in der Statusleiste) ist neu und braucht
> das Gerät. Rückmeldung vom 06.09.2026: Kopieren und Darstellung bestätigt,
> das Symbol fehlt weiterhin — nicht am `roundIcon`, das ist ausgeschlossen;
> dafür gefunden, dass jede Prüf-APK anders signiert ist (Backlog **119**,
> Entscheidung) — und Backlog **120** als Angebot (Live Updates).
...
| 6 | **S4 — Rest** | Kopplungsmodul, feste Server-Adresse, App-Name, Insets, Herkunft je Einsatz (R64), Gerätetest, **Play Console nach R65** (interner Test-Track für Handy und Uhr, Versionscode-Versatz, Signaturweg), Android 1.0.0 | Schritt 5 | Konzept S4, Abschnitt 13 | Opus | **gebaut und gemergt** (Web 14.2.2, Android 0.13.0; PR #33): Teil A, Teil B (R64 und Nr. 63) und Teil C (Play-Console-Vorbereitung) sind fertig; dazu **Android 0.13.1** (Zweig `claude/android-ui-bugs-uc1gcv`) mit vier Befunden vom ersten Blick auf das S24 am 05.09.2026 — Zweierwahl, Zurück-Geste, Code kopieren, Symbol in der Leiste. **Offen:** Gerätetest am S24 und mit der Wear-OS-Uhr (Backlog 81 und 117, Abschnitt 6), Android 1.0.0 nach E-R45-7, D-U-N-S und Signaturschlüssel für die Play Console — und der Push auf `main`. **Rückmeldung 06.09.2026** (nur Doku, Android bleibt 0.13.1): Code kopieren und Darstellung bestätigt, Symbol weiterhin nicht — Backlog 117 (Prüfweg 2), 119 (Prüf-Signatur, Entscheidung), 120 (Angebot) |
...
benennen (Stylesheet, Stammdaten- und Einstellungsseiten). P5 setzt S9
...
**Ziel:** v1.0 sieht aus wie v1.0. **Inhalt:** Umbenennung überall (Web und
Handbuch heißen noch „Einsatzdoku", die Uhr seit 2.0.0 „NAdoku"), neues
Demo-Passwort mit dem Produktnamen in der Schwachwortliste (R25) ·
...
| 77 | Wartungsseite `update.php` in Unterseiten aufteilen | S8 | Schnitt im Konzept; **Unterseite „Migrationen" zeigt nur Ausstehende mit „Ausstehende ausführen", der Torwächter liest dasselbe Register (R66)**; ausgeführte Kennungen bis P5 eingeklappt, danach im Audit-Protokoll; der Wartungsmodus-Schalter (Paket W) zieht auf die Unterseite „Serverbetrieb" |
...
| 117 | Kein Symbol in der Statusleiste während der Aufzeichnung (S24) | Gerätetest, Schritt 6 | mit Android 0.13.1 getan, was am Code dafür in Frage kommt; **06.09.2026 vom Gerät: weiterhin nicht da** — die Manifest-Änderung von 0.11.1 ist als Ursache ausgeschlossen (dasselbe Drawable mit und ohne `roundIcon`), der Meldungsweg ist seit 0.7.7 unverändert; Prüfweg in zweiter Fassung im Punkt selbst (sechs Fragen), zusammen mit Nr. 81 |
...
| 119 | Jede Prüf-APK trägt eine andere Signatur (Debug-Schlüssel je Container) | Schritt 6, Teil C — Entscheidung des Auftraggebers | drei Wege im Punkt, Empfehlung: eigener Prüfschlüssel im Repositorium; bis dahin ist jede Prüf-APK eine Neuinstallation mit zurückgesetzten Berechtigungen und Kategorien |
| 120 | Dauerpräsenz der Aufzeichnung in der Leiste über „Live Updates" (Android 16, Now Bar) | Entscheidung des Auftraggebers nach Nr. 117 | Angebot, kein Beschluss; als eigenes Paket in Schritt 6, wenn gewollt |
...
| Was | Wofür | Wann |
|---|---|---|
| **Zweiter Blick auf das S24 mit Android 0.13.1** — die sechs Punkte im Nachtrag zu Abschnitt 4 des Prüfdokuments S4: Zweierwahl, Zurück-Geste, Code kopieren, Symbol in der Leiste (Nr. 117), Kopf der Benachrichtigung (Nr. 81), Startbildschirm-Symbol auf Handy und Uhr. Erst dieser Blick schließt 81 und 117. **Rückmeldung 06.09.2026:** Code kopieren und Darstellung bestätigt, das Symbol fehlt weiterhin — und die eine Frage, die die Suche halbiert, ist unbeantwortet: Steht die Meldung in der heruntergezogenen Liste? Dazu die Fassung von Android und One UI (Backlog 117, Prüfweg zweite Fassung, sechs Fragen) | Schritt 6 | jetzt, mit der vorhandenen 0.13.1 — keine neue Fassung nötig |
| **Entscheidung: ein fester Prüfschlüssel** (Backlog 119) — Prüf-APKs aus verschiedenen Sitzungen sind heute verschieden signiert, jede Auslieferung ist eine Neuinstallation. Drei Wege im Backlog, Empfehlung (a): Prüfschlüssel im Repositorium, benannte Ausnahme in `.gitignore` | Schritt 6, Teil C | vor der nächsten Prüf-APK |
...
| **28** | **05.09.2026** | **Android 0.13.1 — vier Befunde vom ersten Blick auf das S24** (Zweig `claude/android-ui-bugs-uc1gcv`, nur `android/` und Doku): Die Zweierwahl füllte ihre Zeile nicht (`fillMaxHeight()` gegen unendliche Höhe in einer rollenden Spalte — der Bilderlauf hatte es in 72 Bildern und maß es nicht, weil er nur farbige Knöpfe misst; jetzt `ZweierwahlBildTest`); die Zurück-Geste beendete aus den Einstellungen die App (kein angemeldeter Rückruf; `BackHandler`); der Kopplungscode ließ sich weder markieren noch kopieren (Knopf „Code kopieren" und `SelectionContainer`); in der Statusleiste erscheint während der Aufzeichnung kein Symbol mehr — hier nicht nachstellbar, Themenattribut am Meldungssymbol ausgetragen, und die App sagt jetzt, wenn ihre Benachrichtigungen abgeschaltet sind (**Backlog 117**, Prüfweg am Gerät). **Dabei Backlog 81 am Code gefunden:** fester 52-dp-Vordergrund im adaptiven Symbol, richtig nur bei 108 dp; jetzt Bruchteile, `SymbolBildTest`. Gegenprobe am S24 für 81 und 117 offen (Prüfdokument S4, Abschnitt 4) |
| **29** | **06.09.2026** | **Rückmeldung vom S24 zu Android 0.13.1, nur Dokumentation:** Code kopieren und Darstellung bestätigt; das Symbol in der Leiste fehlt weiterhin, der Auftraggeber sieht die Ursache im Logo-Fix (0.11.1, `roundIcon`). Am Code nachgeprüft und ausgeschlossen — dasselbe Drawable mit und ohne Attribut, Meldungsweg seit 0.7.7 unverändert, Meldungs-IDs ohne Kollision; **kein Placebo ins nächste APK.** Gefunden stattdessen: Jede Prüf-APK trägt den Debug-Schlüssel ihres Containers, jede Auslieferung war eine Neuinstallation (**Backlog 119**, Entscheidung). Prüfweg zu 117 in zweiter Fassung (sechs Fragen); **Backlog 120** als Angebot (Live Updates, Now Bar) |
