...
**Fassung 36 (07.09.2026)** — Neustrukturierung (Fassung 16). Dieses Dokument steuert
...
15.6.0 (elf Punkte, elf Commits) und Android 0.14.0 (Nr. 142–145 und der
Räumteil von 114, fünf Commits), Zweig `claude/sofortpaket-sicherheit-1t70p1`
gepusht. Offen ist der **Merge auf `main`** nach Freigabe; wer von 9a und S9
zweiter mergt, zieht die Buchführung nach (K7). Das Prüfdokument liegt als
...
Nr. 150): Der Menüpunkt „Rettungsmittel" entfällt, „Standorte" wird Liste
...
Punkte, elf Commits, Zweig `claude/sofortpaket-sicherheit-1t70p1` gepusht,
...
**Offen:** der Android-Teil (Nr. 142–145, Räumteil 114) und danach der Merge
auf `main`. Nr. 140 bleibt im Backlog offen — die Wache steht, Branch-Schutz
und 2FA sind Zuarbeit, das Deploy-Tor ist S10. Aus Nr. 135 ist **Nr. 153**
herausgelöst (`querySelector` mit einem Wert aus dem URL-Fragment).
...
| 137 | Photon und Kachelserver (K-6) | **S9** (AP2, seit 07.09.2026) | Hinweis am Feld, Datenschutztext, Installationsschalter (F-SP-4), Kontoschalter, Dienstadresse — alles E-S9-05; Kachelserver bleiben (SP-12); Selbstbetrieb bleibt R36 |
...
`docs/konzepte/Pruefdokument-Sofortpaket-Sicherheit.md`. **Web, elf Punkte,
elf Commits:** Rundenzahl 600 000 mit stiller Anhebung und Passwortregeln
...
1 abweichend; Android — `./gradlew build` grün — Handy **261 Prüffälle je Bauart** (Debug und Release; vorher 247), **0 Fehlschläge**, 15 übersprungen (14 Rundlauf ohne Installation und der jeweils bauartfremde Fall aus Nr. 142); Uhr **71 Prüffälle**, 0 übersprungen; Lint **0 Fehler** (Handy 13 Warnungen, unverändert die `libs.versions.toml`-Hinweise; Uhr 0); Release-APK Handy **7 867 394 B** (+332 B gegen 0.13.0), Uhr **19 574 406 B** (unverändert); Bilderlauf 72 Bilder wie zuvor, Emulator **nicht erreicht** — Stufe II steht für 0.14.0 aus. Drei Startversuche mit `-accel off`, `-memory 6144`, Abbild `android-34;default;x86_64`, Emulator 37.1.11: Der erste stand nach 14 min bei `adb devices` = `device`, noch ohne `sys.boot_completed`, und wurde mit dem Abbruch der wartenden Shell mitgerissen; der zweite lief 38 min bei 102 % eines Kerns (die ersten 10 min parallel zum vollen Baulauf, Last 8 auf 4 Kernen) und blieb bei `device offline`; der dritte läuft seit 16:07 Uhr auf leerer Maschine. Befund mit Zahl statt stillschweigend übersprungener Punkt (CLAUDE.md 6); nachholen nach Prüfliste P-13, Wortliste
...
Auftraggeber. *Letzter Commit:* `f2f177a` (Nr. 145) plus der Buchführungs-Commit dieser Fassung. Merge auf `main` nach Freigabe.
