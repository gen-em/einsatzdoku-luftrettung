...
**Fassung 105 (23.09.2026)** — dieses Dokument steuert das Programm:
...
| **Prüfliste TB — P-TB-06 und P-TB-07** (`docs/konzepte/Pruefdokument-TB-Tor-nach-Baum.md`). **P-TB-06:** mit **Web 20.37.2** (eigens dafür, `server/` sonst unverändert) Schritt 7 des Produktionslaufs lesen — er muss einen PR-Lauf mit passendem Baum und grünem Job `Stufe 1` nennen und darf nicht „Kein grüner Stufe-1-Lauf" melden. **Ein Tag ohne passende Fassung belegt das nicht:** `web-v20.37.1-test` (Lauf 35844281706) und `web-v20.37.2` auf `eefffca` (Lauf 35845324185) endeten an „Tag gegen WEB_VERSION halten", einen Schritt vor dem Tor. **Vor dem Tag den alten `web-v20.37.2` löschen.** **P-TB-07:** beim nächsten Fall zweier offener PRs „Update branch" nach dem ersten Merge sehen | Konzept TB | Tag `web-v20.37.2` nach dem Merge bzw. nächster Fall |
