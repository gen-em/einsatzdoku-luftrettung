...
Backlog Nr. 11 und Nr. 14 (beide P4), Nr. 13 (P6 nach R46) und Nr. 47/48 aus
...
Uhr-Punkten bleibt allein **Backlog Nr. 46** (Serverseite der
...
Bestätigungsseite mit Art und Modell (Backlog Nr. 46), Meldungen für
...
**Backlog:** Nr. 46 (Serverseite der Gerätekennung, R42-Kleinstpaket) ist
...
mitgenommen, weil für Nr. 47 ohnehin eine Uhr-Auslieferung anstand.
...
R49; R50 seit Fassung 14). Stand Fassung 14: **das Kleinstpaket und S2
sind in Arbeit.** Das Kleinstpaket (R42 und R44) läuft **vor** S2 als
eigene Kleinauslieferung nach dem Muster R20 — es ist keine Phase, hat kein
Konzept und kein Prüfdokument, sondern besteht aus den beiden R-Einträgen.
Es steht vorn, weil beide Punkte mit jedem Tag Wartezeit teurer werden: Die
Gerätekennung fehlt jeder Kopplung, die vorher stattfindet (R42).
...
Migration verlangt danach `update.php`). **Jetzt parallel zu R42-Kleinstpaket
und S2:** die Konzeptarbeit S4, das Konzept S3 ist fertig, die Vorarbeiten zu
...
| S2 | **fertig** (`docs/Konzept-S2-Mengen-Spuren-Sicherung.md`; E-S2-01 bis E-S2-24, Beschlüsse 28.–30.08.2026) | **in Arbeit** (seit 31.08.2026) | Zwischenpaket nach R34. Elf Pakete AP0–AP10; Migrationen (`track_blobs`, Job-Tabelle, Admin-Einstellungen) legt die Umsetzung fest (K3). Kein Fable-Schritt. Neue Prüfmittel: Messstand (R35), Fassung-4-Referenz. Zuarbeiten: Transport-Testziele, SMTP-Bestätigung |
...
| S3 | offen (R43; Grundlage `ToDo_Layout.pdf` vom 31.08.2026, 19 Punkte) — **ohne offene Gestaltungsfrage**, E-R43-1 bis E-R43-3 entschieden | offen | Zwischenpaket **nach S2**, vor P5, parallel zu P4 führbar. Kern: Anwendungsregel zur vorhandenen Abstandsskala in `docs/Design.md`, Umsetzung an Klassen und Bausteinen (**kein Flickwerk**). Enthält einen echten Fehler mit geklärter Ursache (Markerversatz, `geo.js`/`.geo-schild`). Kein Fable-Schritt. **Zweite Rückmeldungsrunde (01.09.2026), zwei Punkte vorab umgesetzt** und auf einem eigenen Zweig committet, noch nicht ausgeliefert: das Dateifeld der Sicherungsseite (nativer Knopf klebte am oberen Rand — gemessen 0 px Luft darüber, 19 px darunter; behoben am Baustein `input[type=file].feld-eingabe`, nicht an der Seite, also im Sinne des „kein Flickwerk" von R43) und der fehlende Hinweis, dass die erzeugte Datei heruntergeladen wurde (Sicherung und Datenexport, je mit Dateinamen). Beide stehen **nicht** in `ToDo_Layout.pdf` — die Liste hat 19 Punkte am Stand 9.14.1, diese sind neuer; sie gehören ins S3-Konzept nachgetragen. Stilvergleich 35 763 Elementmessungen über 13 Breiten, 13 Abweichungen (ein Element, eine Eigenschaft, keine darüber hinaus). **Der Zweig konfliktiert inzwischen mit S2/AP5 in `einstellungen.php`** — dieselbe Erfolgsmeldung, beide Seiten erweitern sie; wird nach dem S2-Merge aufgelöst (Anweisung 01.09.2026) |
...
| Zugangsdaten je eines echten FTP-, FTPS- und SFTP-Ziels (Testverzeichnis genügt) | S2, Abnahme der Transportadapter (AP7) | vor AP7 |
| Bestätigung, dass SMTP auf der Produktivinstallation eingerichtet ist | S2, Warnmails (AP6) | vor der AP6-Abnahme |
