...
**Fassung 54 (14.09.2026)** — dieses Dokument steuert das Programm:
...
| 183 | Der Pruefstand kennt nur eine Engine | **Zuarbeit** (Allowlist), Mittel spaeter | Aufgenommen 14.09.2026 bei der Gegenprobe zu Nr. 182. Jede Browserpruefung laeuft ueber Playwright, und das hat hier **nur Chromium** — seit Web 19.4.1 haengt eine Darstellung an einer Container-Abfrage, seit P3 an `:has()` und `dvh`. **Zwei von drei Engines sind belegt** (Chromium 141, WebKitGTK 2.52.6 aus den Paketquellen); **Gecko fehlt**, weil `cdn.playwright.dev` und `playwright.download.prss.microsoft.com` mit 403 gesperrt sind. Billigste Loesung: zwei Eintraege in der Egress-Allowlist, keine Codearbeit |
