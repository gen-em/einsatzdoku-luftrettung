...
| 4b | **Töne gegen den Wertevorrat** (P5b/AP9, Backlog Nr. 36) — jeder Wert, den `ui_plakette`, `ui_knopf`, `ui_kennzahl` oder `ui_meldung_markup` bekommt, hat eine Regel im Stylesheet. Die Klassenprüfung (1) kann das nicht: `ui_plakette()` setzt den Namen erst zur Laufzeit zusammen (`'plakette-' . $ton`), also steht `plakette-ok` weder im Markup noch im Stylesheet. Derselbe Fehler ist viermal aufgetreten — `warn`, `ok` zweimal, `info`. **Grenze:** nur Literale; ein `'ton' => $ton` wird übersprungen. |
