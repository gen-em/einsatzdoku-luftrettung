...
| fremde Quelle | `config.example.php` | `einsatz.example.invalid` | **Beispieltext in der Vorlage.** `config.example.php` wird nicht ausgeliefert und nicht ausgeführt; sie zeigt, wie `base_url` aussieht. **Hier stand bis PK-04/5c `example.de`, mit der Begründung, das sei „eine reservierte Beispieldomäne" — das ist falsch.** RFC 2606 reserviert `example.com`, `.net`, `.org` und die TLDs `.invalid`, `.test`, `.example`, `.localhost`; `example.de` ist registrierbar und kann jemandem gehören. Die Vorlage richtete damit auch SMTP (`mail.example.de`, `noreply@example.de`) auf eine fremde Domäne. Alle vier Stellen stehen jetzt auf `.invalid`. |
...
| fremde Quelle | `assets/export.js` | `www.topografix.com` | **XML-Namensraum, keine Adresse — dieselbe Kennung wie `GPX_NS` darüber**, geschrieben vom GPX-Export im Browser. Bis BV-01 (Backlog Nr. 184) sah die Prüfung sie nicht: Ein `"` in einem Regex-Literal weiter oben (`csvEscape()`) brachte den Kommentar-Abtaster aus dem Tritt, und er hielt das `//` in dieser Zeichenkette für einen Kommentaranfang. Ein falsches Negativ, wie Nr. 184 es beschreibt — hier harmlos, weil die Adresse nie abgerufen wird. |
...
| Seite ohne Gerüst | `doku_seite.php` | `ui_seite_start` | **Handbuch und Was-ist-NAdoku** (P5b/AP8, E-P5b-08). Derselbe Fall wie eine Zeile darüber: Beide Seiten sind ohne Anmeldung erreichbar, und das ist der Zweck — ein Handbuch hinter der Anmeldung hilft genau denen nicht, die nicht hineinkommen. Statt des Gerüsts trägt `hilfe.php` ein eigenes Inhaltsverzeichnis (`.doku-nav`), das zum Dokument gehört und nicht zur Anwendung. |
| Seite ohne Gerüst | `adresse_bestaetigen.php` | `ui_seite_start` | **Ohne Anmeldung, und das ist der Zweck** (P5b/AP5, E-P5b-16). Wer den Link aus dem Postfach der NEUEN Adresse anklickt, führt damit den Nachweis, um den es geht — eine Anmeldung zusätzlich zu verlangen, bewiese nichts und wäre gerade dann unmöglich, wenn sie am meisten hülfe. Keine Sitzung, kein Gerüst. |
| Seite ohne Gerüst | `registrieren.php` | `ui_seite_start` | **Vor dem ersten Konto** (P5b/AP3, E-P5b-13). Wer sich registriert, hat weder Sitzung noch Diensttag noch Menü — die Seite ist die Nachbarin der Anmeldung, mit derselben dunkelblauen Fläche und derselben Karte. |
| Seite ohne Gerüst | `bestaetigen.php` | `ui_seite_start` | **Die Seite nach der Registrierung** (P5b/AP3). Sie erscheint unmittelbar nach dem Festlegen des Passworts und damit VOR der ersten Anmeldung; bei der Betriebsart „mit Freischaltung" kann bis dahin noch eine Frist vergehen. |
| Seite ohne Gerüst | `einwilligung.php` | `ui_seite_start` | **Ein Tor, kein Ziel** (P5b/AP4, E-P5b-05). Hier ist eine Sitzung da, das Gerüst fehlt trotzdem mit Absicht: Seine Bereichsnavigation führte auf Seiten, die gerade gesperrt sind, und ein Menü, dessen Einträge alle auf dieselbe Seite zurückwerfen, ist schlimmer als keines. Die drei Wege, die offen bleiben müssen — abmelden, ausleiten, löschen —, stehen deshalb ausgeschrieben auf der Seite selbst. |
| Seite ohne Gerüst | `zweitfaktor.php` | `ui_seite_start` | **Ein Tor, kein Ziel — wie `einwilligung.php`** (P5c/AP5, E-P5c-61). Eine Pflichtrolle ohne Zweitfaktor erreicht keine andere Seite; ein Menü, dessen Einträge alle hierher zurückwerfen, wäre schlimmer als keines. Die Seite steht in der Anmeldehülle, weil sie zur Anmeldung gehört: Erst mit dem Zweitfaktor ist die Anmeldung fertig. Offen bleibt nur das Abmelden, ausgeschrieben auf der Seite. |
