...
`update.php`. Die drei Betriebsseiten stehen deshalb in der Ausnahmeliste —
ohne den Eintrag antwortete ausgerechnet die Seite mit dem Ausschalter mit 503
(F-S8-P-04). Fall 6 misst genau das, für alle drei.
...
| 2 offen | `betrieb_updates.php` mit BetreiberIn-Sitzung → 200 mit Balken und Ausschalt-Knopf, `betrieb_jobs.php` und `betrieb_server.php` ebenso → 200 (6) · `betrieb_updates.php` mit Nutzer-Sitzung → 403 wie sonst, **nicht** 503, und `update.php` als Übergangsseite → 200 (7) · `jobs.php` mit gültigem Token → 200, die Jobs laufen (8, E-S5W-11) · mit falschem Token → 403 `token` (9) · `login.php` → 200 mit Balken **und** Formular (10) · `wiederherstellen.php` mit Admin-Sitzung → nicht 503 (11) · `assets/style.css` → 200 (12) |
...
| Fall 6 rot mit HTTP 503 | Eine der drei Betriebsseiten fehlt in `WARTUNG_AUSNAHMEN`. Die Seite mit dem Ausschalter sperrt sich dann selbst aus (F-S8-P-04) |
