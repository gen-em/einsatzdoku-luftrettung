...
503 (F-S8-P-04). Fall 6 misst genau das, für alle fünf. Seit S10 kommt
**`betrieb_schluesselblatt.php`** als sechste dazu (Fall 6a): Wer den
Server-Anteil vom Blatt abliest, tut das in einer Wartungslage — hinter der
Sperre stünde der Wert genau dann nicht zur Verfügung, wenn man ihn braucht.
Es ist die einzige Ausnahmeseite **ohne** Wartungsbalken, und das ist Absicht:
Sie hat kein Gerüst, weil sie gedruckt wird. Fall 6a sagt das ausdrücklich, damit
niemand sie „der Vollständigkeit halber" in die Balkenprüfung nimmt. `update.php` selbst
...
## Was sie prüft — 57 Erwartungen in sieben Teilen
...
| 3 Schalten | Ausschalten über `betrieb_updates.php` (POST, CSRF) → Datei weg, Startseite antwortet wieder; Einschalten → Datei da, mit Zeitpunkt und Konto (13) · `wartung.lock` mit kaputtem Inhalt → **trotzdem 503**, Balken „seit unbekannt" (14) · das 503 kommt schneller als die Antwort ohne Wartung — das Tor greift vor Datenbank und Ratenschutz (15). **Diese eine Erwartung flackert** (Backlog Nr. 172): Sie vergleicht zwei Einzelmessungen ohne Spielraum, und auf der lokalen Anlage liegen beide bei rund 71 ms — gemessen 0, 1, 0 nicht erfüllte in drei Läufen hintereinander. Wer hier rot sieht, prüfe zuerst, ob es diese Nummer ist |
| 4 Kommandozeile und Regeln | `php update.php` läuft im Wartungsmodus (16, Notausgang) · die Ausnahmeliste ist **genau** die aus E-S5W-04 plus die fünf Betriebsseiten aus S8/AP2 und AP4 plus `auth_salt.php` aus Nr. 171 plus `betrieb_schluesselblatt.php` aus S10 — **dreizehn** Einträge (17) · `login.php` liest `role`, prüft es seit S8/AP1 über `rolle_darf_verwalten()` und verwirft im Wartungsmodus die Sitzung ohne Verwaltungsrecht, und zwar **erst nach** `rate_erfolg` (18, E-S5W-09) |
...
**Die Zahl, die zählt, steht in der letzten Zeile:** `-> 57 Erwartungen, 0
nicht erfuellt`.
...
| Fall 6a rot mit HTTP 503 | `betrieb_schluesselblatt.php` fehlt in `WARTUNG_AUSNAHMEN`. Dann steht der Server-Anteil hinter genau der Tür, die man ohne ihn nicht aufbekommt (S10) |
| Fall 17 rot | Die Ausnahmeliste ist gewachsen oder geschrumpft, ohne dass die Erwartung mitgezogen wurde — **genau das ist zwischen S10/AP3 und AP5 passiert**: dreizehn Einträge gegen zwölf gezählte, und die Probe stand unbemerkt auf 1 nicht erfüllt |
